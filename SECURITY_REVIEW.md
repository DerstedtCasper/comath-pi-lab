...
- Phase 41-51 execute live adapters through the scheduler boundary, add capped log readback plus cursor-based log polling, SSE-compatible subscription snapshots, bounded multi-event SSE log-session responses, read-only operator panels, same-process scheduler-backed cancellation, and bounded health probes, add a service-owned packaged adapter launcher, support service-configured external Codex-compatible CLI invocation, and add a service-configured Codex API backend contract. Production Codex API account/network validation, production installed-CLI validation, indefinite WebSocket/SSE sessions beyond bounded responses, richer operator controls beyond same-process cancellation, OS-level sandboxing, and enforced network denial remain deferred.
