...
Task146-150 extend this lifecycle line with a fail-closed Pi/Codex readiness gate, Pi release consumer, artifact-backed lifecycle evidence intake, a service-owned durable `comathd` lifecycle probe, and a service-owned production Codex API account/network validation probe. These artifacts remain release-readiness evidence with `proof_authority=none`; they do not certify GA, replace real Pi host install/runtime-registration validation, provide indefinite operator sessions, or provide OS-enforced adapter isolation.
