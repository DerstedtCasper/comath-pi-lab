...
Task235 adds a macOS `sandbox-exec` production-helper profile contract. Provider-runner, helper-host-validation, helper-execution, and provider-helper collection manifests must carry a service-derived `production_helper_profile_contract` for macOS helpers that binds the helper profile source, helper binary hash, macOS `sandbox-exec` facility family, disabled network policy, no-new-privileges requirement, no `sandbox-exec` command execution requirement for the profile contract, no macOS sandbox profile/policy inspection, and `proof_authority="none"`. This is audit lineage only: it must not be treated as shipped production helper coverage, OS-enforcement proof, readiness evidence by itself, real-Pi evidence, Lean proof authority, broad provider support, or GA certification.
