...
- Provider host capability probe manifests, discovered host tools, platform/kernel feature facts, probe exit status, diagnostics, or route payloads can be treated as provider-helper readiness, helper self-test/runtime-attestation evidence, executed OS isolation, collected OS-enforcement evidence, mathematical proof authority, real-Pi execution evidence, broad provider support, or GA certification.
