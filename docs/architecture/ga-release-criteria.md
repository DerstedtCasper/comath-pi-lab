...
- Configured provider-helper asset environment variables, including `COMATH_AGENT_ADAPTER_OSISO_WINDOWS_APPCONTAINER_HELPER` and `COMATH_AGENT_ADAPTER_OSISO_WINDOWS_APPCONTAINER_HELPER_ARGS_JSON`, can be treated as executed OS isolation, readiness-review evidence, broad provider support, mathematical proof authority, real-Pi execution, or GA certification.
- Adapter OS-isolation provider-helper host-validation manifests, helper-host-ready booleans, helper binary hashes, supported-platform lists, or route payloads can be treated as executed OS isolation, readiness-review evidence, broad provider support, mathematical proof authority, or GA certification.
- Adapter OS-isolation provider-helper execution manifests, missing/unvalidated/mismatched host-validation bindings, helper args-prefix hash/count metadata, helper exit codes, stdout/stderr/transcript hashes, or route payloads can be treated as collected OS-enforcement evidence, readiness-review evidence, broad provider support, mathematical proof authority, or GA certification.
- Adapter OS-isolation provider-helper collection route payloads, caller-supplied collection booleans, caller-supplied stdout/stderr/transcript hashes, or wrapper manifests can be treated as collected OS-enforcement evidence without an internal service-owned collection callback and canonical Task170 probe/evidence artifact.
