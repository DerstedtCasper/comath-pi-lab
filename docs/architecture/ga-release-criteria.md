...
- Adapter OS-isolation provider-helper collection route payloads, caller-supplied collection booleans, caller-supplied stdout/stderr/transcript hashes, helper executions without current runtime-attestation binding, hash-bound service-owned collection callbacks with incomplete OS-enforcement facts, or wrapper manifests can be treated as collected OS-enforcement evidence without an internal service-owned collection callback, complete process/filesystem/network/no-new-privileges/escape-prevention checks, and canonical Task170 probe/evidence artifact.
