...
- Task235 keeps a macOS `sandbox-exec` production-helper profile contract under release-hardening coverage: macOS helper manifests must carry a service-derived `production_helper_profile_contract` binding helper profile source, helper executable hash, macOS `sandbox-exec` facility family, disabled network policy, no-new-privileges requirement, path-free host-facility tool name, and non-authority flags. The focused suite `goal3-task235-agent-adapter-os-isolation-macos-sandbox-exec-production-helper-profile-contract.test.mjs` must keep this lineage distinct from `sandbox-exec` command execution, profile/policy inspection, OS-enforcement proof, readiness, Lean authority, real-Pi evidence, broad provider support, and GA certification.
...
node services/comathd/tests/unit/goal3-task235-agent-adapter-os-isolation-macos-sandbox-exec-production-helper-profile-contract.test.mjs
