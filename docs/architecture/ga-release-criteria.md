...
- Adapter OS-isolation provider-helper collection route payloads, caller-supplied collection booleans, caller-supplied stdout/stderr/transcript hashes, helper executions without current runtime-attestation binding, default route collection blockers, hash-bound service-owned collection probes with incomplete OS-enforcement facts, or wrapper manifests can be treated as collected OS-enforcement evidence without complete process/filesystem/network/no-new-privileges/escape-prevention checks and a collected canonical Task170 probe/evidence artifact.
