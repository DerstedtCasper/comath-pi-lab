...
| Product must not be a rough scaffold. | `README.md` describes Phase 18-74 product surfaces; `TODO.md` marks Phases 60-74 complete; `REVIEW.md` records Phase 60-74 evidence. | In scope and product-real for registered slices plus fail-closed planning/repair/target-package/proof-body/authority-preview evidence. | Continue one task per Goal 2 turn until final audit can prove completion. |
...
| Broad theorem planning/synthesis has a product slice beyond registered templates. | `phase70-broad-theorem-planning-slice.test.mjs` verifies non-template planning artifacts; Phase 72 adds one bounded theorem-specific target package; Phase 73 adds a bounded non-authoritative proof-body candidate; Phase 74 adds bounded static/statement/dependency/axiom preview reports while blocking final replay/promotion. | Implemented as fail-closed planning plus one target-package/proof-body/report-preview slice. | Arbitrary proof synthesis remains deferred. |
...
| One theorem-specific Lean target/proof-body/report-preview candidate exists beyond broad planning. | `phase72-theorem-specific-lean-generation.test.mjs` verifies the bounded target package; `phase73-bounded-lean-proof-body-synthesis.test.mjs` verifies proof-body synthesis; `phase74-bounded-authority-report-preparation.test.mjs` verifies `bounded_authority_report_preparation.json`, static/statement/dependency/axiom preview reports, no final replay manifest, and unchanged conjectural claim status. | Implemented as non-authoritative target-package plus proof-body plus authority-preview evidence. | Final clean replay and arbitrary theorem proving remain deferred. |
...
| --- | --- | --- | --- |
| Broad proof planning and theorem synthesis beyond the Phase 70 fail-closed planning slice, Phase 72 bounded target package, Phase 73 bounded proof-body candidate, Phase 74 bounded authority-preview reports, registered theorem families, and exact refutation. | Global-GA limitation. | Current product writes replayable planning evidence for non-template targets and one theorem-specific Lean proof-body candidate with preview reports, but does not synthesize arbitrary proof bodies or final proofs. | Blocks arbitrary-theorem claims; does not invalidate current Phase 18-74 evidence. |
