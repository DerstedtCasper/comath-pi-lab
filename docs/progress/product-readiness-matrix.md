...
| Product must not be a rough scaffold. | `README.md` describes Phase 18-78 product surfaces; `TODO.md` marks Phases 60-78 complete; `REVIEW.md` records Phase 60-78 evidence. | In scope and product-real for registered slices plus fail-closed planning/repair/target-package/proof-body/authority-preview/final-replay evidence, runner service-level network-denial policy, and registered transitive statement-equivalence witness chains. | Continue one task per Goal 2 turn until final audit can prove completion. |
...
| Lean Authority v2 controls `formally_checked` promotion. | Phase 31-37, 54, 56, 57, 64, and 78 tests; final replay hash binding; statement equivalence, dependency closure, axiom profile, static audit, and clean replay artifacts. | Implemented for registered slices with fail-closed binding, including direct and transitive registered equivalence metadata. | Broader proof-search-grade equivalence and deeper Lake/mathlib provenance remain deferred. |
...
| Richer statement equivalence beyond registered aliases/witnesses. | Mathematical limitation. | Current equivalence is conservative, target-bound, and witness-backed, including direct registered witnesses and explicitly registered closed transitive witness chains. | Blocks automatic semantic-equivalence/proof-search claims. |
...
- Statement equivalence handles automatically discovered or arbitrary semantic equivalence beyond registered witness chains.
