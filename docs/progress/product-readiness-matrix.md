...
| Product must not be a rough scaffold. | `README.md` describes Phase 18-79 product surfaces; `TODO.md` marks Phases 60-79 complete; `REVIEW.md` records Phase 60-79 evidence. | In scope and product-real for registered slices plus fail-closed planning/repair/target-package/proof-body/authority-preview/final-replay evidence, runner service-level network-denial policy, registered transitive statement-equivalence witness chains, and non-authoritative equivalence-search plan artifacts. | Continue one task per Goal 2 turn until final audit can prove completion. |
...
| Richer statement equivalence beyond registered aliases/witnesses and unresolved plan artifacts. | Mathematical limitation. | Current equivalence is conservative, target-bound, and witness-backed where accepted; Phase 79 can write blocked proof-search obligations for unresolved mismatches but does not execute them. | Blocks automatic semantic-equivalence/proof-execution claims. |
...
- Statement equivalence handles automatically discovered or arbitrary semantic equivalence beyond registered witness chains and non-authoritative blocked plan artifacts.
