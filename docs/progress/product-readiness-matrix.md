...
| Product must not be a rough scaffold. | `README.md` describes Phase 18-73 product surfaces; `TODO.md` marks Phases 60-73 complete; `REVIEW.md` records Phase 60-73 evidence. | In scope and product-real for registered slices plus fail-closed planning/repair/target-package/proof-body candidate evidence. | Continue one task per Goal 2 turn until final audit can prove completion. |
...
| Broad theorem planning/synthesis has a product slice beyond registered templates. | `phase70-broad-theorem-planning-slice.test.mjs` verifies non-template planning artifacts; Phase 72 adds one bounded theorem-specific target package; Phase 73 adds a bounded non-authoritative proof-body candidate and static audit preview while blocking final replay/promotion. | Implemented as fail-closed planning plus one target-package/proof-body-candidate slice. | Arbitrary proof synthesis remains deferred. |
...
| One theorem-specific Lean target/proof-body candidate exists beyond broad planning. | `phase72-theorem-specific-lean-generation.test.mjs` verifies the bounded target package; `phase73-bounded-lean-proof-body-synthesis.test.mjs` verifies `bounded_proof_body_synthesis.json`, `bounded_proof_body_static_audit.json`, target/replay binding, negative prompt guard, no final replay manifest, and unchanged conjectural claim status. | Implemented as non-authoritative target-package plus proof-body candidate evidence. | Authority reports, final clean replay, and arbitrary theorem proving remain deferred. |
...
| --- | --- | --- | --- |
| Broad proof planning and theorem synthesis beyond the Phase 70 fail-closed planning slice, Phase 72 bounded target package, Phase 73 bounded proof-body candidate, registered theorem families, and exact refutation. | Global-GA limitation. | Current product writes replayable planning evidence for non-template targets and one theorem-specific Lean proof-body candidate, but does not synthesize arbitrary proof bodies, authority reports, or proofs. | Blocks arbitrary-theorem claims; does not invalidate current Phase 18-73 evidence. |
