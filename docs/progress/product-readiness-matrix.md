...
The current product claim is bounded: CoMath Pi Lab is a local auditable research workbench at Research Alpha plus Phase 18-68 GA/v3 vertical-slice implementation. It is not final global v3 GA readiness until Task 15 completes the requirement-by-requirement audit, and it is not an arbitrary theorem prover, not a MathProve proof-authority system, not a production Codex/Pi managed service, and not an OS-sandboxed execution platform.
...
| Product must not be a rough scaffold. | `README.md` describes Phase 18-68 product surfaces; `TODO.md` marks Phases 60-68 complete; `REVIEW.md` records Phase 60-68 evidence. | In scope and product-real for registered slices. | Task 15 must verify all final requirements and current docs after this sync. |
...
| ResearchCampaign drives bounded resumable work through `comathd`. | Phase 20 state machine; Phase 60 pause/tick guard; Phase 63 v3 stage gates; Phase 66 Pi goal-compatible UX; Phase 67 formal campaign slice. | Implemented for supported product slices. | Terminal vocabulary aliases from external v3 docs remain deferred in `TODO.md`. |
...
| Security and mathematical-integrity reviews are current. | `SECURITY_REVIEW.md`, `MATH_INTEGRITY_REVIEW.md`, `docs/architecture/risk-register.md`, and `docs/architecture/acceptance-matrix.md` now include Phase 68 boundaries. | Synchronized for Task 14. | Task 15 must run final scans and audit before goal completion. |
...
| Full v3 external-document terminal vocabulary compatibility. | API compatibility limitation. | Current public API uses canonical internal terminal names while the external docs name alternate terminal vocabulary. | Must remain documented until aliases/adapters are implemented. |
