...
| Product must not be a rough scaffold. | `README.md` describes Phase 18-75 product surfaces; `TODO.md` marks Phases 60-75 complete; `REVIEW.md` records Phase 60-75 evidence. | In scope and product-real for registered slices plus fail-closed planning/repair/target-package/proof-body/authority-preview/final-replay evidence. | Continue one task per Goal 2 turn until final audit can prove completion. |
...
| Broad theorem planning/synthesis has a product slice beyond registered templates. | `phase70-broad-theorem-planning-slice.test.mjs` verifies unsupported non-template planning artifacts; Phase 72 adds one bounded theorem-specific target package; Phase 73 adds a bounded proof-body candidate; Phase 74 adds bounded static/statement/dependency/axiom preview reports; Phase 75 runs final clean replay and promotion for that bounded target. | Implemented as fail-closed planning plus one bounded target-package/proof-body/report/final-replay slice. | Arbitrary proof synthesis remains deferred. |
...
| One theorem-specific Lean target/proof-body/report/final-replay candidate exists beyond broad planning. | `phase72-theorem-specific-lean-generation.test.mjs` verifies the bounded target package; `phase73-bounded-lean-proof-body-synthesis.test.mjs` verifies proof-body synthesis; `phase74-bounded-authority-report-preparation.test.mjs` verifies preview reports; `phase75-bounded-final-clean-replay.test.mjs` verifies claim-scoped final reports, hash-bound replay manifest, and `formally_checked` promotion. | Implemented for one bounded non-template target. | Arbitrary theorem proving remains deferred. |
...
| --- | --- | --- | --- |
| Broad proof planning and theorem synthesis beyond the Phase 70 fail-closed planning slice, Phase 72-75 bounded final-replay path, registered theorem families, and exact refutation. | Global-GA limitation. | Current product writes replayable planning evidence for unsupported non-template targets and proves one bounded `n + n = 2 * n` target, but does not synthesize arbitrary proof bodies or final proofs. | Blocks arbitrary-theorem claims; does not invalidate current Phase 18-75 evidence. |
