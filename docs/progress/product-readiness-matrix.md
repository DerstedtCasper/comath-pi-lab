...
| Product must not be a rough scaffold. | `README.md` describes Phase 18-71 product surfaces; `TODO.md` marks Phases 60-71 complete; `REVIEW.md` records Phase 60-71 evidence. | In scope and product-real for registered slices and fail-closed planning/repair evidence. | Continue one task per Goal 2 turn until final audit can prove completion. |
...
| ResearchCampaign drives bounded resumable work through `comathd`. | Phase 20 state machine; Phase 60 pause/tick guard; Phase 63 v3 stage gates; Phase 66 Pi goal-compatible UX; Phase 67 formal campaign slice; Phase 69 terminal vocabulary projection; Phase 71 stage-gate repair/resume. | Implemented for supported product slices. | Broad theorem generalization remains deferred. |
...
| --- | --- | --- | --- |
| Broad proof planning and theorem synthesis beyond the Phase 70 fail-closed planning slice, registered theorem families, and exact refutation. | Global-GA limitation. | Current product now writes replayable planning evidence for non-template targets, but does not generate arbitrary theorem-specific Lean projects or proofs. | Blocks arbitrary-theorem claims; does not invalidate current Phase 18-70 evidence. |
