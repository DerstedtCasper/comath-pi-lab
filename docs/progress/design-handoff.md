...
- Phase 77 service-level runner network-denial environment policy;
- Phase 78 registered transitive statement-equivalence witnesses;
- Phase 79 non-authoritative statement-equivalence proof-search plan artifacts;
- Phase 80 bounded equivalence-search witness materialization metadata;
...
The Phase 45 local install-session e2e, Phase 41-53 live/profile/Codex adapter slices, Phase 70 broad-planning evidence, Phase 71 stage-gate repair/resume, Phase 72 bounded Lean target package, Phase 73 bounded proof-body candidate, Phase 74 bounded authority-preview reports, Phase 75 bounded final clean replay promotion, Phase 76 registered Nat linear identity target table, Phase 77 runner network policy, Phase 78 registered transitive statement-equivalence witness chains, Phase 79 blocked equivalence-search plan artifacts, and Phase 80 bounded materialized equivalence-witness metadata have retired older descriptor-only, "no broad target evidence", "no bounded proof-body/report preview", "bounded target lacks final replay", "single hardcoded bounded target", "no runner network policy", "no transitive registered witness chain", "no unresolved equivalence plan artifact", and "no bounded equivalence witness materialization" blockers. Do not start broad generalization implementation without keeping the active GA goal and validation trail explicit. The next phase should retire current global GA blockers--arbitrary theorem synthesis beyond the registered bounded target/proof-body/report/final-replay slice, broad MathProve proof search or proof-authority semantics, stronger OS/network runner sandboxing, richer real-host Pi UX/service lifecycle management, production Codex API account/network validation, indefinite operator sessions, cross-process scheduler recovery, and automatic statement-equivalence proof execution beyond registered bounded materializations--not merely add another documentation slice. Research Alpha and Phase 18-80 validation evidence is recorded in `REVIEW.md`.
...
- Phase 25 adds `runMathProveBridgeExternal()` for controlled `MathProve-Skill` `verify_sympy.py` evidence runs; Phase 58 adds `runMathProveFinalAuditExternal()` for controlled `MathProve-Skill` `final_audit.py` evidence runs. These extend evidence production but do not change the proof-authority boundary.
...
- Added `StatementRegisteredTransitiveLogicalEquivalence` and `allowed_registered_transitive_logical_equivalences` to the Lean statement-equivalence gate.
- Accepted `registered_transitive_logical_equivalence` witnesses only when the chain endpoint binds the locked formal spec to the extracted target signature, every intermediate link closes exactly, and every link carries `lean_kernel_checked_equivalence`, witness artifact id, valid SHA-256 witness artifact hash, and non-empty lemma names.
- Broken endpoints, missing links, non-kernel witness kinds, missing hashes, and missing lemma names remain hard statement-mismatch vetoes.
- Added `services/comathd/tests/unit/phase78-lean-transitive-equivalence.test.mjs` to the default `@comath/comathd` test chain and status capability `lean_registered_transitive_logical_equivalence_witnesses`.
- Remaining statement-equivalence hardening after Phase 80: automatic proof search/execution beyond registered bounded materializations and broader mathematical-domain trust profiles.

## Phase 79 Completion Notes

- Added `StatementEquivalenceSearchPlan` and optional `equivalence_search_plan_path` / `equivalence_search_hints` inputs to the Lean statement-equivalence gate.
- When a target signature is unique but mismatched, no exact/alias/direct/transitive registered witness accepts it, and safe lemma-name hints are provided, the gate writes `equivalence_search_plan.json` with `result: blocked_unproved`, `proof_authority: none`, and `can_promote_claim: false`.
- Exact, direct registered, transitive registered, missing-output, empty-hint, and unsafe-hint paths do not write unresolved plans.
- Added `services/comathd/tests/unit/phase79-lean-equivalence-search-plan.test.mjs` to the default `@comath/comathd` test chain and status capability `lean_equivalence_search_plan_artifacts`.
- Remaining statement-equivalence hardening after Phase 80: automatic proof search/execution beyond registered bounded materializations and broader mathematical-domain trust profiles.

## Phase 80 Completion Notes

- Added `materializeStatementEquivalenceSearchPlan()` for service-owned bounded materialization of a Phase 79 blocked plan into registered logical-equivalence witness metadata.
- Materialization requires `blocked_unproved`, `proof_authority: none`, `can_promote_claim: false`, exact formal-spec/target binding, safe lemma hints, a non-empty witness artifact id, and a bounded allowlisted materialization.
- Wrote `equivalence_witness_materialized.json` with SHA-256 materialization binding, `proof_authority: none`, `can_promote_claim: false`, and required final Lean Authority v2 gates.
- Added `services/comathd/tests/unit/phase80-bounded-equivalence-witness-materialization.test.mjs` to the default `@comath/comathd` test chain and status capability `lean_equivalence_witness_materialization`.
- Remaining statement-equivalence hardening: automatic semantic-equivalence discovery/proof execution beyond registered bounded materializations and broader mathematical-domain trust profiles.
...
## Phase 58 Completion Notes

- Added `runMathProveFinalAuditExternal()` for controlled external invocation of the sibling `MathProve-Skill` `scripts/final_audit.py` with CoMath-generated steps JSON, a controlled workspace, bounded timeout, `shell:false`, `network=false` metadata, host-path scrubbing, and replay/steps/solution/script/stdout/stderr/result hashes.
...
- The final-audit bridge is runner evidence only: it still emits authority vetoes, keeps `gate_result=failed`, and cannot promote claims without CoMath proof-kernel replay.
