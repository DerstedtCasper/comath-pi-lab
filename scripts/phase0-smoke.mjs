...
const required = [
  ...
  "docs/progress/production-formal-workbench-retrospective.md",
...
const schemaReadme = readFileSync(join(root, "schemas/README.md"), "utf8");
...
if (!readme.includes("Production Formal Workbench Core") || !readme.includes("not GA")) {
  invariantFailures.push("README must describe the current Production Formal Workbench Core, not GA state");
}
...
const requiredSections = [
  ...
  [acceptanceMatrix, "## Production Formal Workbench Acceptance", "acceptance matrix must define production formal workbench acceptance"],
  [acceptanceMatrix, "## Mathematical Integrity Acceptance", "acceptance matrix must define math integrity acceptance"],
  [schemaReadme, "formal-proof-run.schema.json", "schema README must list production formal proof run schema"],
  [schemaReadme, "mathprove-run-manifest.schema.json", "schema README must list MathProve run manifest schema"],
