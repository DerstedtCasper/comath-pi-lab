...
- Research Alpha plus Phase 18-27 is a local auditable prototype with narrow executable proof-kernel slices and an AgentRun contract boundary, not a production arbitrary theorem prover.
- Generic proof planning, broad MathProve proof search/final-audit semantics, a real child-agent launcher/scheduler, native TriviumDB performance validation, full DLP-grade secret scanning, full interactive Pi/comathd install-session e2e, and OS/network sandboxed runner replay remain later candidates.
