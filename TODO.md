...
## Production Formal Workbench P1-P6

### P1: Formal Proof Authority

- [x] Add formal proof Zod contracts and root JSON schema coverage.
- [x] Add append-only formal proof run store under `.comath/evidence/formal-proof-runs.jsonl`.
- [x] Require `formally_checked` promotion to bind to a trusted Lean4 kernel-checked formal proof run for the same project, claim, and proof/log artifacts.
- [x] Reject kernel-checked formal proof runs containing `sorry` or `admit`.
- [x] Split formalization, dependency closure, and proof audit into independent certification provenance records before final claim certification.
- [x] Keep raw formal proof run append and raw gate-promoted claim writers out of the public barrel API.
- [x] Keep raw gate decision application out of the public barrel API.
- [x] Reject caller-supplied Lean executables as formal proof authority; service-resolved Lean4 is the only execution path.
- [x] Keep MathProve as evidence producer and manifest source; it cannot promote claims or substitute for Lean kernel evidence.

### P2: Provenance Ledger And Paper Spans

- [x] Add append-only provenance ledger under `.comath/provenance/events.jsonl`.
- [x] Record production event kinds including `formal_proof.run_recorded`, `mathprove.run_recorded`, `paper.span_recorded`, and `index.rebuilt`.
- [x] Add paper span storage under `.comath/artifacts/papers/spans.json`.
- [x] Extend paper checks so span-to-margin-note integrity is a first-class blocker.

### P3: MathGraphIndex And Trivium Derived Index

- [x] Add `MathGraphIndex` as the memory/index facade over `ResearchMemoryDB`.
- [x] Expose `health()` with backend, `truth_source=provenance-ledger`, `derived_index=true`, degraded state, and rebuildability.
- [x] Add `rebuildFromLedger()` and provenance event recording for index rebuilds.
- [x] Keep TriviumDB optional and derived; native IDs remain hidden behind adapter boundaries.

### P4: Pi Official Adapter

- [x] Add adapter from internal descriptors to official Pi registration shape with `parameters` and `execute(toolCallId, params, signal, onUpdate, ctx)`.
...
- [x] Run external real-model validation through Pi provider config, provider `comath-local`, and model `官方/deepseek-v4-pro`; repository validation intentionally does not commit local Pi/VCP credentials or redirects.

### P5: Session Lock And Mutation Queue

- [x] Add per-project lock under `.comath/session/lock.json`.
- [x] Add mutation queue under `.comath/session/mutations.jsonl`.
- [x] Reject concurrent active lock acquisition.
- [x] Require explicit stale-lock recovery and owner-matching release.
- [x] Acquire locks with exclusive file creation rather than read-then-write.
- [x] Serialize stale-lock recovery with a recovery mutex so competing recovery attempts cannot delete a fresh lock.
- [x] Add `lock_id` capability tokens so owner strings alone cannot release active locks.
- [x] Clean up recovery sentinels even when release observes malformed lock state.
- [x] Use collision-resistant mutation queue IDs rather than read-then-sequential allocation.
- [x] Use collision-resistant append-only IDs for audit, provenance, and gate-result ledgers.
- [x] Add claim mutation compare-and-swap protection so stale promotion/formal-proof metadata writers cannot resurrect older mathematical statements.
- [x] Hash and size-check promotion artifacts from the same open file descriptor.

### P6: Docs, Audits, Final Verification

- [x] Update `TODO.md`, `REVIEW.md`, `SECURITY_REVIEW.md`, and `MATH_INTEGRITY_REVIEW.md` for production formal workbench boundaries.
- [x] Add `docs/progress/production-formal-workbench-retrospective.md`.
- [x] Update `docs/architecture/acceptance-matrix.md`.
- [x] Load runtime `jsonSchemas` from root schema artifacts and test equality with `schemas/*.schema.json`.
- [x] Record final fresh validation: `corepack pnpm build`, `corepack pnpm typecheck`, `corepack pnpm test`, and repository-root `.comath` absence.

## Known Deferred Items

- [ ] Full MathProve workspace runner integration beyond the fail-closed bridge manifest.
- [x] Production Pi extension runtime registration with real-model proof evidence through Pi's own provider config and the local 6005 model endpoint.
...
- [ ] Package/release artifact readiness: semver, license/notice, files/exports/bin, changelog, dry-run pack artifacts, checksums, and artifact provenance.
- [ ] DLP-grade release scanning covering text, binary, archive, PDF, generated artifacts, and remediation records.
- [ ] Reviewed external runtime evidence bundle with `schema_version`, reviewer signoff, evidence artifact paths, and SHA-256 verification.
- [ ] Multi-process locking stress on the target Windows filesystem, including crash recovery, stale recovery, duplicate-ID checks, and lost-update checks.
- [ ] Formal dependency-closure auditing beyond the current conservative metadata and artifact-bound proof-run gate.
