...
- [ ] Broader live Lean/mathlib positive-matrix replay beyond the Task 86 environment-gated PM-084 real replay slice. Default tests must continue to fail closed without claiming Lean authority when the real Lean toolchain is not explicitly enabled, and Task 87 now hard-blocks injected replay callbacks from producing final Lean Authority evidence.
