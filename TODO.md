...
- [ ] Broader live Lean/mathlib positive-matrix replay beyond the Task 86 environment-gated PM-084 real replay slice, Task 88 non-authoritative replay-attempt archive, Task 89 non-authoritative environment diagnostic archive, Task 90 final-authority provenance gate, Task 91 final replay artifact-kind/audit-provenance gate, and Task 92 Pi dashboard audit-evidence semantics fix. Default tests must continue to fail closed without claiming Lean authority when the real Lean toolchain is not explicitly enabled, Task 87 hard-blocks injected replay callbacks from producing final Lean Authority evidence, Task 88 archives replay blockers/pass observations only as non-promotional audit evidence, Task 89 records Lean/Lake version-probe blockers without letting injected probe metadata produce final authority, Task 90 requires append-only final replay registry provenance before helper-created authority-shaped artifacts can satisfy promotion-grade Lean evidence, Task 91 requires that registry provenance be audit-bound and that submitted FinalReplayManifest v3 artifacts be `runner_output`, and Task 92 ensures Pi dashboard consumers render archive/audit evidence as `audit` rather than generic `runner` evidence.
