...
- [ ] Broader live Lean/mathlib positive-matrix replay beyond the Task 86 environment-gated PM-084 real replay slice and Task 88 non-authoritative replay-attempt archive. Default tests must continue to fail closed without claiming Lean authority when the real Lean toolchain is not explicitly enabled, Task 87 hard-blocks injected replay callbacks from producing final Lean Authority evidence, and Task 88 archives replay blockers/pass observations only as non-promotional audit evidence.
