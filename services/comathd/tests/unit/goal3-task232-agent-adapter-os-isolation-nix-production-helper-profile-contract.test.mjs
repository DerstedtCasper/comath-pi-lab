...
function createNixHelperScript(projectRoot) {
  const helperScript = join(projectRoot, "task232-nix-provider-helper.mjs");
  writeFileSync(
    helperScript,
    [
...
function assertNixProfileContract(contract, source, helperSha256, message) {
  assert.ok(contract, `${message}: production helper profile contract must be present`);
  assert.equal(contract.contract_source, "service_owned_nix_sandbox_production_helper_profile_contract");
  assert.equal(contract.provider, "nix_sandbox");
  assert.equal(contract.provider_family, "nix_sandbox");
  ...
  assert.deepEqual(contract.required_host_facility_tools, ["nix_cli", "nix_store_cli"]);
  assert.equal(contract.runtime_family, "nix_develop_no_network");
  ...
  assert.equal(contract.store_or_profile_inspection_allowed, false);
  assert.equal(contract.nix_command_execution_required_for_profile_contract, false);
...
const nixHelperEnvVar = "COMATH_AGENT_ADAPTER_OSISO_NIX_HELPER";
const nixHelperArgsEnvVar = "COMATH_AGENT_ADAPTER_OSISO_NIX_HELPER_ARGS_JSON";
...
const previousNixHelper = process.env[nixHelperEnvVar];
const previousNixHelperArgs = process.env[nixHelperArgsEnvVar];
...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task232-nix-production-helper-profile-"));
...
try {
  assert.equal(
    ...
    true,
    "Task232 capability ledger must advertise the Nix production-helper profile contract without claiming GA"
  );

  const focusedTestName = "goal3-task232-agent-adapter-os-isolation-nix-production-helper-profile-contract.test.mjs";
  ...
  for (const [path, pattern] of [
    ["README.md", /Task232.*Nix.*production-helper profile contract/s],
    ["AGENTS.md", /Task232.*Nix.*production-helper profile contract/s],
    ["TODO.md", /Task213-232 summary/s],
    ["REVIEW.md", /Goal 3 Task 232/s],
    ["config/README.md", /Task232.*Nix.*production-helper profile contract/s],
    ["docs/architecture/adapter-contracts.md", /Task232.*Nix.*production-helper profile contract/s],
    ["docs/architecture/threat-model.md", /Task232.*Nix.*production-helper profile contract/s]
  ]) {
    assert.match(readRepoFile(path), pattern, `${path} must document Task232 without overclaiming`);
  }
  const osIsolationSource = readRepoFile("services/comathd/src/agents/agent-adapter-os-isolation.ts");
  assert.equal(
    osIsolationSource.includes('hostValidation.provider !== "oci_container"'),
    false,
    "host-validation executable gates must require valid profile contracts for every provider with a profile spec, not only OCI"
  );
  assert.equal(
    osIsolationSource.includes('helperExecution.provider !== "oci_container"'),
    false,
    "helper-execution collectability gates must require valid profile contracts for every provider with a profile spec, not only OCI"
  );
  const sampleConfig = JSON.parse(readRepoFile("config/comath.sample.json"));
  const nixProvider = sampleConfig?.agentAdapterOsIsolation?.providerHelpers?.find(
    (entry) => entry?.provider === "nix_sandbox"
  );
  assert.equal(nixProvider?.productionHelperProfileContractRequired, true);
  assert.equal(nixProvider?.productionHelperProfileContractKind, "nix_sandbox_develop_profile_contract");
  assert.deepEqual(nixProvider?.productionHelperProfileSources, [
    "operator_configured_provider_helper",
    "bundled_provider_helper_protocol_asset"
  ]);
  assert.deepEqual(nixProvider?.requiredHostFacilityTools, ["nix_cli", "nix_store_cli"]);

  const helperScript = createNixHelperScript(projectRoot);
  process.env[nixHelperEnvVar] = process.execPath;
  process.env[nixHelperArgsEnvVar] = JSON.stringify([helperScript]);
  ...
  const init = initProject({ name: "Goal3 Task232 Nix Production Helper Profile", root_path: projectRoot });
  const projectId = init.project.project_id;
  const launch = prepareAgentAdapterOsIsolationSandboxLaunch(projectRoot, {
    project_id: projectId,
    launch_id: "ADAPTER-OSISO-LAUNCH-0232-NIX",
    adapter_id: "codex-cli",
    backend: "external",
    actor: `${projectRoot} token=launch-secret`,
    requested_provider: "nix_sandbox",
    ...
    launcher_probe: () => ({
      ...
      launcher_version: "nix-launcher-task232",
  ...
  const runner = prepareAgentAdapterOsIsolationProviderRunner(projectRoot, {
    project_id: projectId,
    runner_id: "ADAPTER-OSISO-RUNNER-0232-NIX",
    ...
    runner_environment: {
      ...
      nix_store_path: `${projectRoot}/nix/store`
    }
  });
  assert.equal(runner.ok, true);
  assert.equal(runner.provider, "nix_sandbox");
  ...
  assert.equal(JSON.stringify(runner).includes("nix/store"), false);

  const hostCapability = probeAgentAdapterOsIsolationProviderHostCapability(projectRoot, {
    project_id: projectId,
    host_capability_probe_id: "ADAPTER-OSISO-HOST-CAP-0232-NIX",
    adapter_id: "codex-cli",
    backend: "external",
    ...
    provider_host_capability_probe: () => ({
      ...
      capability_facts: ["task232 nix sandbox host capability observed"],
      required_tools: [
        { name: "nix_cli", present: true, version: null, binary_sha256: helperSha256 },
        { name: "nix_store_cli", present: true, version: null, binary_sha256: helperSha256 }
      ],
      kernel_features: ["task232-nix-host-capability"],
      diagnostics: [`${projectRoot} host capability diagnostic must be scrubbed`, "Nix capability observed"]
    })
  });
  const nixSupportedHost = process.platform === "linux" || process.platform === "darwin";
  assert.equal(hostCapability.ok, nixSupportedHost);
  ...
  assert.equal(
    hostCapability.provider_host_capability.required_tools.some((tool) => tool.name === "nix_cli"),
    true
  );
  assert.equal(
    hostCapability.provider_host_capability.required_tools.some((tool) => tool.name === "nix_store_cli"),
    true
  );
  ...
  if (!nixSupportedHost) {
    ...
    const hostValidator = ({ host_validation_id }) => ({
      ...
      helper_version: "nix_sandbox-helper-task232",
      ...
      self_test_transcript_sha256: sha256Text(`task232:${host_validation_id}`),
      self_test_args_prefix_sha256: sha256Text("task232-nix-prefix"),
      self_test_args_prefix_count: 1,
      self_test_fixed_args_sha256: sha256Text("task232-nix-fixed-args"),
      diagnostics: [`${projectRoot} host validator diagnostic must be scrubbed`, "Nix helper host validated"]
    });

    const host = validateAgentAdapterOsIsolationProviderHelperHost(projectRoot, {
      project_id: projectId,
      host_validation_id: "ADAPTER-OSISO-HELPER-HOST-0232-NIX",
    ...
    delete process.env[nixHelperEnvVar];
    delete process.env[nixHelperArgsEnvVar];
    const driftExecution = runAgentAdapterOsIsolationProviderHelperExecution(projectRoot, {
      project_id: projectId,
      helper_execution_id: "ADAPTER-OSISO-HELPER-0232-NIX-DRIFT",
    ...
    assert.equal(
      driftExecution.ok,
      false,
      "helper execution must fail closed if the Nix helper profile source drifts after host validation"
    );
    ...
    const helperExecution = runAgentAdapterOsIsolationProviderHelperExecution(projectRoot, {
      project_id: projectId,
      helper_execution_id: "ADAPTER-OSISO-HELPER-0232-NIX",
      host_validation_id: host.host_validation_id,
      runner_id: runner.runner_id,
      launch_id: launch.launch_id,
      adapter_id: "codex-cli",
    ...
    const collection = collectAgentAdapterOsIsolationProviderHelperExecutionEvidence(projectRoot, {
      project_id: projectId,
      collection_id: "ADAPTER-OSISO-HELPER-COLLECT-0232-NIX",
  ...
  if (previousNixHelper === undefined) {
    delete process.env[nixHelperEnvVar];
  } else {
    process.env[nixHelperEnvVar] = previousNixHelper;
  }
  if (previousNixHelperArgs === undefined) {
    delete process.env[nixHelperArgsEnvVar];
  } else {
    process.env[nixHelperArgsEnvVar] = previousNixHelperArgs;
  }
...
console.log("Goal 3 Task232 Nix production-helper profile contract tests passed.");
