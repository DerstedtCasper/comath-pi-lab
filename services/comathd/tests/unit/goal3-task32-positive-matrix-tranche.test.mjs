...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task32-positive-tranche-"));
...
assert.equal(tranche.tranche_scope.start_task_id, "PM-079");
assert.equal(tranche.tranche_scope.end_task_id, "PM-089");
...
assert.deepEqual(
  ...
  ["PM-079", "PM-080", "PM-081", "PM-082", "PM-083", "PM-084", "PM-085", "PM-086", "PM-087", "PM-088", "PM-089"]
);
assert.equal(tranche.summary.clean_replay_passed, 0, "Task 32 must not fake clean replay for PM-079 through PM-089");
assert.equal(tranche.summary.replayable_blocker, 11);
assert.equal(tranche.summary.promoted_count, 0);
assert.equal(tranche.next_batch_scope.start_after_task_id, "PM-089");
assert.equal(tranche.next_batch_scope.remaining_tasks, 11);
...
for (const taskId of ["PM-081", "PM-082", "PM-083", "PM-084", "PM-085", "PM-086", "PM-087", "PM-088", "PM-089"]) {
  assert.equal(byTask.get(taskId).category, "theorem-search-assisted", `${taskId} must stay in the theorem-search-assisted tranche segment`);
}
...
for (const result of tranche.results) {
  ...
  if (result.category === "paper-to-formal-spec") {
...
assert.equal(
  existsSync(join(projectRoot, ".comath/release/positive_matrix/PM-090/replay_attempt_certificate.json")),
  false,
  "Task 32 tranche must not write the next-batch PM-090 certificate"
);

console.log("Goal 3 Task 32 positive matrix tranche tests passed.");
