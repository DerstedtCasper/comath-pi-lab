...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-lean-authority-v2-"));
...
try {
  const { project } = initProject({ name: "Lean Authority v2 Final Gate", root_path: projectRoot });
  ...
  const staleReplayManifestPath = writeProjectFile(
    join(".comath", "evidence", claim.id, "lean", "final_replay_manifest.json"),
    `${JSON.stringify(
      {
        replay_id: "RPLY-0001",
  ...
  const replayArtifact = await importArtifact({
    projectRoot,
    project_id: project.project_id,
    source_path: staleReplayManifestPath,
    kind: "runner_output",
    actor: "phase64"
  });
  const evidence = appendEvidenceRecord(projectRoot, {
    ...
    claim_id: claim.id,
    kind: "lean",
    summary: "A stale old-format final replay manifest is present but lacks Lean authority v2 hash-bound reports.",
    artifact_ids: [proofArtifact.id, replayArtifact.id]
  });
  ...
  assert.equal(
    promotion.gate.vetoes.includes("formally_checked requires hash-bound fresh final replay artifacts"),
    true
  );
  ...
  const reportPaths = {
    ...
    statement_equivalence: writeProjectFile(
      ...
      JSON.stringify({ result: "pass", equivalence: "exact", hard_vetoes: [] })
    )
  };
  const boundReplayManifestPath = writeProjectFile(
    join(".comath", "evidence", claim.id, "lean", "final_replay_manifest_bound.json"),
    `${JSON.stringify(
      {
        replay_id: "RPLY-0002",
        campaign_id: "CAM-0001",
        ...
        primary_dependency: "Nat.add_zero",
        locked_statement_hash: claim.statement_hash,
        clean_workspace_path: ".comath/lean/final_replay/RPLY-0002/clean",
        lean_toolchain: "leanprover/lean4:v4.23.0",
        lakefile_hash: "a".repeat(64),
        local_file_hashes: {
          "MathResearch/C0001.lean": "b".repeat(64)
  ...
  const boundReplayArtifact = await importArtifact({
    projectRoot,
    project_id: project.project_id,
    source_path: boundReplayManifestPath,
    kind: "runner_output",
    actor: "phase64"
  });
  writeProjectFile(`.comath/evidence/${claim.id}/lean/final_replay.log`, "stale log replaced after manifest import\n");
  const staleEvidence = appendEvidenceRecord(projectRoot, {
    project_id: project.project_id,
    claim_id: claim.id,
    kind: "lean",
    summary: "A hash-bound final replay manifest exists, but its live replay log has drifted after import.",
    artifact_ids: [proofArtifact.id, boundReplayArtifact.id]
  });
  ...
  assert.equal(
    stalePromotion.gate.vetoes.includes("formally_checked requires hash-bound fresh final replay artifacts"),
    true
  );
} finally {
...
console.log("Phase 64 Lean authority v2 final gate tests passed.");
