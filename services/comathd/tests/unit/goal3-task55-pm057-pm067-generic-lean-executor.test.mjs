...
const selected = manifest.tasks.filter((task) => task.task_id >= "PM-057" && task.task_id <= "PM-067");
assert.equal(selected.length, 11, "PM-057 through PM-067 must form an 11-task bounded tranche");

const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task55-tranche-"));
...
assert.equal(trancheReport.start_task_id, "PM-057");
assert.equal(trancheReport.end_task_id, "PM-067");
...
assert.equal(trancheReport.tranche_report_path, ".comath/release/positive_matrix/PM-057_PM-067/lean_authority_executor_tranche_v1.json");
assert.equal(replayCommands.length, 22, "each PM-057 through PM-067 task should attempt check and build through the generic executor");
...
for (const result of trancheReport.results) {
  assert.match(result.task_id, /^PM-05[7-9]$|^PM-06[0-7]$/);
...
assert.equal(
  existsSync(join(projectRoot, ".comath/release/positive_matrix/PM-068/lean_authority_executor_blocker_v3.json")),
  false,
  "Task 55 tranche must not write next-batch PM-068 executor artifacts"
);

console.log("Goal 3 Task 55 PM-057 through PM-067 generic Lean executor test passed.");
