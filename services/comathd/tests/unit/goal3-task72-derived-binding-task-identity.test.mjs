...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task72-task-identity-"));
...
try {
  const { project } = initProject({ name: "Goal 3 Task 72 Task Identity", root_path: projectRoot });
  const requestedTaskId = "PM-072";
  const foreignTaskId = "PM-070";
  const claimStatement = "theorem Goal3Positive072 : True";
  ...
  const replayId = "RPLY-0072";
  const runId = "LRUN-0072";
  ...
  const target = writeProjectFile(
    ...
    [
      ...
      "",
      "theorem Goal3Positive072 : True := by",
      "  trivial",
      "",
      "#check Goal3Positive072",
      "#print axioms Goal3Positive072",
      "",
      "end MathResearch",
      ""
    ].join("\n")
  );
  const audit = writeProjectFile(`${cleanRootRel}/Audit/TargetAudit.lean`, "import MathResearch.Target\n#check MathResearch.Goal3Positive072\n#print axioms MathResearch.Goal3Positive072\n");
  ...
  const stdout = writeProjectFile(`.comath/evidence/${claim.id}/lean/${runId}.stdout.log`, "Goal3Positive072 checked\n");
  ...
  const finalReplayManifest = createFinalReplayManifestV3({
    ...
    campaign_id: "CAM-0072",
    claim_id: claim.id,
    theorem_name: "MathResearch.Goal3Positive072",
  ...
  const bindingArtifact = await importArtifact({
    projectRoot,
    project_id: project.project_id,
    source_path: `.comath/release/positive_matrix/${requestedTaskId}/derived_final_authority_bindings_v3.json`,
    kind: "runner_output",
    actor: "goal3-task72"
  });
  const evidence = appendEvidenceRecord(projectRoot, {
    ...
    claim_id: claim.id,
    kind: "lean",
    summary: "Cross-task derived binding material must fail closed.",
  ...
  assert.equal(rejected.gate.ok, false, "foreign PM binding material must not satisfy the ordinary promotion gate");
} finally {
  rmSync(projectRoot, { recursive: true, force: true });
}

console.log("Goal 3 Task 72 derived binding task identity test passed.");
