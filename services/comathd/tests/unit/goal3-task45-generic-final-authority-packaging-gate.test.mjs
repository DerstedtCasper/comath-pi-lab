...
import {
  ...
  importArtifact,
  initProject,
  packageFinalAuthorityEvidenceV3,
...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task45-generic-authority-"));
...
try {
  const { project } = initProject({ name: "Goal 3 Task 45 Generic Authority", root_path: projectRoot });
  ...
  const leanBinary = writeProjectFile(`${cleanRootRel}/bin/lean`, "dummy lean executable for Task45 binary provenance\n");
  const lakeBinary = writeProjectFile(`${cleanRootRel}/bin/lake`, "dummy lake executable for Task45 binary provenance\n");
  ...
  appendFinalReplayRegistryEntryV3(projectRoot, finalReplayManifest, { project_id: project.project_id, actor: "goal3-task45" });
  const replayPack = writeThirdPartyReplayPackV3(projectRoot, finalReplayManifest);

  const pm002Packaging = packageGoal3GaPm002FinalAuthorityEvidence({
    projectRoot,
    materialSource: {
      ...
      lean_run_manifest_id: "LRUN-0045",
  ...
  const genericPackaging = packageFinalAuthorityEvidenceV3({
    ...
    taskId: "PM-045",
    claimId: claim.id,
    sourceReport: pm002Packaging
  });
  assert.equal(genericPackaging.schema_version, "comath.final_authority_packaging.v3");
  assert.equal(genericPackaging.task_id, "PM-045");
  assert.equal(genericPackaging.claim_id, claim.id);
  assert.equal(genericPackaging.final_evidence_status, "verified_final_authority_evidence");
  assert.equal(genericPackaging.can_promote_claim, false);
  assert.equal(genericPackaging.promotion_requires_gate, true);

  const packagingArtifact = await importArtifact({
    projectRoot,
    project_id: project.project_id,
    source_path: genericPackaging.packaging_report_path,
    kind: "runner_output",
    actor: "goal3-task45"
  });
  ...
  const leanEvidence = appendEvidenceRecord(projectRoot, {
    ...
    claim_id: claim.id,
    kind: "lean",
    summary: "Verified generic final authority packaging plus FinalReplayManifest v3 for ordinary promotion-gate review.",
  ...
  assert.equal(packagingAlone.gate.ok, false, "generic packaging evidence must not bypass claim gate prerequisites");
...
console.log("Goal 3 Task 45 generic final authority packaging gate test passed.");
