...
try {
  ...
  const firstAuditEvent = appendAuditEvent(projectRoot, {
    project_id: project.project_id,
    event_type: "phase3.first_append_only_event",
    actor: "phase3-test",
    payload: { note: "append-only ledgers use collision-resistant ids" }
  });
  assert.match(firstAuditEvent.id, /^AUD-\d{4,}$/);
  assert.notEqual(firstAuditEvent.id, "AUD-0001", "append-only audit IDs must not be read-count allocated");
  ...
  writeFileSync(secretSource, "SECRET=this-is-a-placeholder-not-a-real-credential\n", "utf8");
