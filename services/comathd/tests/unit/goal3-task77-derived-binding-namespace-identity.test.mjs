...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task77-namespace-identity-"));
...
try {
  const { project } = initProject({ name: "Goal 3 Task 77 Namespace Identity Binding", root_path: projectRoot });
  const taskId = "PM-077";
  const claimStatement = "theorem Goal3Positive077 : True";
  ...
  const replayId = "RPLY-0077";
  const runId = "LRUN-0077";
  ...
  const target = writeProjectFile(
    ...
    [
      ...
      "",
      "theorem Goal3Positive077 : True := by",
      "  trivial",
      "",
      "#check Goal3Positive077",
      "#print axioms Goal3Positive077",
      "",
      "end MathResearch",
      ""
    ].join("\n")
  );
  const audit = writeProjectFile(`${cleanRootRel}/Audit/TargetAudit.lean`, "import MathResearch.Target\n#check MathResearch.Goal3Positive077\n#print axioms MathResearch.Goal3Positive077\n");
  ...
  const formalSpec = writeProjectFile(formalSpecRel, `${JSON.stringify({
    ...
    theorem_name: "Goal3Positive077",
    theorem_header: "theorem Goal3Positive077 : True",
  ...
  const stdout = writeProjectFile(`.comath/evidence/${claim.id}/lean/${runId}.stdout.log`, "Goal3Positive077 checked\n");
  ...
  const finalReplayManifest = createFinalReplayManifestV3({
    ...
    campaign_id: "CAM-0077",
    claim_id: claim.id,
    theorem_name: "MathResearch.Goal3Positive077",
  ...
  const evidence = appendEvidenceRecord(projectRoot, {
    ...
    claim_id: claim.id,
    kind: "lean",
    summary: "Same-theorem-name wrong namespace binding material must fail closed.",
  ...
  assert.equal(rejected.gate.ok, false, "wrong FormalSpecLock namespace must not satisfy the ordinary promotion gate");
} finally {
  rmSync(projectRoot, { recursive: true, force: true });
}

console.log("Goal 3 Task 77 derived binding namespace identity test passed.");
