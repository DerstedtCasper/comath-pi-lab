...
assert.equal(batch.summary.clean_replay_passed, 1, "bounded first batch should include the deterministic v3 replay-bound fixture pass");
assert.equal(batch.summary.replayable_blocker, 99, "all non-passing tasks must remain replayable blockers");
...
const passed = batch.results.find((result) => result.terminal_classification === "clean_replay_passed");
assert.ok(passed, "first bounded batch must produce a replay-bound fixture result");
assert.equal(passed.task_id, "PM-001");
assert.equal(passed.proof_authority, "lean_kernel_clean_replay");
assert.equal(passed.can_promote_claim, false, "Task 21 batch evidence is matrix evidence, not direct claim promotion");
assert.equal(passed.evidence_binding.formal_spec_lock_hash.length, 64);
assert.equal(passed.evidence_binding.assumption_ledger_hash.length, 64);
assert.equal(passed.evidence_binding.dependency_lock_hash.length, 64);
assert.equal(passed.evidence_binding.artifact_hashes_sha256.length, 64);
assert.ok(passed.evidence_binding.lean_run_manifest_id.length > 0);
assert.ok(passed.evidence_binding.final_replay_manifest_id.length > 0);
