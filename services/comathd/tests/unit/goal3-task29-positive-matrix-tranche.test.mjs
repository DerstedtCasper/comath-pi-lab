...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task29-positive-tranche-"));
...
assert.equal(tranche.tranche_scope.start_task_id, "PM-057");
assert.equal(tranche.tranche_scope.end_task_id, "PM-067");
...
assert.deepEqual(
  ...
  ["PM-057", "PM-058", "PM-059", "PM-060", "PM-061", "PM-062", "PM-063", "PM-064", "PM-065", "PM-066", "PM-067"]
);
assert.equal(tranche.summary.clean_replay_passed, 0, "Task 29 must not fake clean replay for PM-057 through PM-067");
assert.equal(tranche.summary.replayable_blocker, 11);
assert.equal(tranche.summary.promoted_count, 0);
assert.equal(tranche.next_batch_scope.start_after_task_id, "PM-067");
assert.equal(tranche.next_batch_scope.remaining_tasks, 33);

const byTask = new Map(tranche.results.map((result) => [result.task_id, result]));
for (const taskId of ["PM-057", "PM-058", "PM-059", "PM-060"]) {
  assert.equal(byTask.get(taskId).category, "combinatorics", `${taskId} must stay in the combinatorics tranche segment`);
}
for (const taskId of ["PM-061", "PM-062", "PM-063", "PM-064", "PM-065", "PM-066", "PM-067"]) {
...
assert.equal(
  existsSync(join(projectRoot, ".comath/release/positive_matrix/PM-068/replay_attempt_certificate.json")),
  false,
  "Task 29 tranche must not write the next-batch PM-068 certificate"
);

console.log("Goal 3 Task 29 positive matrix tranche tests passed.");
