...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task119-lean-manifest-provenance-gate-"));
...
try {
  const { project } = initProject({ name: "Goal 3 Task 119 LeanRunManifest Provenance Gate", root_path: projectRoot });
  const taskId = "PM-099";
  const claimStatement = "theorem Goal3Positive119 : True";
  ...
  const replayId = "RPLY-0119";
  const runId = "LRUN-0119";
  ...
  const target = writeProjectFile(
    ...
    [
      ...
      "",
      "theorem Goal3Positive119 : True := by",
      "  trivial",
      "",
      "#check Goal3Positive119",
      "#print axioms Goal3Positive119",
      "",
      "end MathResearch",
      ""
    ].join("\n")
  ...
  const audit = writeProjectFile(
    ...
    "import MathResearch.Target\n#check MathResearch.Goal3Positive119\n#print axioms MathResearch.Goal3Positive119\n"
  );
  ...
  const leanBinary = writeProjectFile(`${cleanRootRel}/bin/lean`, "dummy lean executable for Task119 binary provenance\n");
  const lakeBinary = writeProjectFile(`${cleanRootRel}/bin/lake`, "dummy lake executable for Task119 binary provenance\n");
  const stdout = writeProjectFile(`.comath/evidence/${claim.id}/lean/${runId}.stdout.log`, "Goal3Positive119 checked\n");
  ...
  const finalReplayManifest = createFinalReplayManifestV3({
    ...
    campaign_id: "CAM-0119",
    claim_id: claim.id,
    theorem_name: "MathResearch.Goal3Positive119",
  ...
  appendFinalReplayRegistryEntryV3(projectRoot, finalReplayManifest, { project_id: project.project_id, actor: "goal3-task119" });
  ...
  const evidence = appendEvidenceRecord(projectRoot, {
    ...
    claim_id: claim.id,
    kind: "lean",
    summary: "Task119 submits final authority evidence whose LeanRunManifest was not bound to the provenance index.",
  ...
  assert.equal(promotion.gate.ok, false, "promotion must reject LeanRunManifest material missing provenance-index binding");
  assert.equal(promotion.claim.status, "conjectural");
  assert.ok(
    promotion.gate.vetoes.includes("formally_checked requires LeanRunManifest provenance index"),
    JSON.stringify(promotion.gate.vetoes)
  );
} finally {
  rmSync(projectRoot, { recursive: true, force: true });
}

console.log("Goal 3 Task 119 Lean manifest provenance promotion gate test passed.");
