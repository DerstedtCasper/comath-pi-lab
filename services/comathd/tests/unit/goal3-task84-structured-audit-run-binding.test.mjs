...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task84-structured-audit-run-"));
...
try {
  const { project } = initProject({ name: "Goal 3 Task 84 Structured Audit Run Binding", root_path: projectRoot });
  const taskId = "PM-084";
  const claimStatement = "theorem Goal3Positive084 : True";
  ...
  const replayId = "RPLY-0084";
  const runId = "LRUN-0084";
  ...
  const target = writeProjectFile(
    ...
    [
      ...
      "",
      "theorem Goal3Positive084 : True := by",
      "  trivial",
      "",
      "#check Goal3Positive084",
      "#print axioms Goal3Positive084",
      "",
      "end MathResearch",
      ""
    ].join("\n")
  );
  const audit = writeProjectFile(`${cleanRootRel}/Audit/TargetAudit.lean`, "import MathResearch.Target\n#check MathResearch.Goal3Positive084\n#print axioms MathResearch.Goal3Positive084\n");
  ...
  const formalSpec = writeProjectFile(formalSpecRel, `${JSON.stringify({
    ...
    claim_id: claim.id,
    namespace: "MathResearch",
    theorem_name: "Goal3Positive084",
    theorem_header: "theorem Goal3Positive084 : True",
  ...
  const stdout = writeProjectFile(`.comath/evidence/${claim.id}/lean/${runId}.stdout.log`, "Goal3Positive084 checked\n");
  ...
  writeProjectFile(structuredAuditRel, `${JSON.stringify({
    result: "pass",
    hard_vetoes: [],
  ...
  const finalReplayManifest = createFinalReplayManifestV3({
    ...
    campaign_id: "CAM-0084",
    claim_id: claim.id,
    theorem_name: "MathResearch.Goal3Positive084",
  ...
  writeProjectFile(dependencyClosureRel, `${JSON.stringify({
    result: "pass",
    hard_vetoes: [],
    tampered_after_final_replay_manifest: true
  })}\n`);
  ...
  const evidence = appendEvidenceRecord(projectRoot, {
    ...
    claim_id: claim.id,
    kind: "lean",
    summary: "Final authority reports must retain the hashes recorded in FinalReplayManifest v3.",
  ...
  assert.equal(rejected.gate.ok, false, "post-manifest report substitution must not satisfy the ordinary promotion gate");
} finally {
  rmSync(projectRoot, { recursive: true, force: true });
}

console.log("Goal 3 Task 84 final-report hash binding test passed.");
