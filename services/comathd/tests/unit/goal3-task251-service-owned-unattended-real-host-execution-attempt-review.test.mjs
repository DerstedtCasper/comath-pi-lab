...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task251-attempt-review-"));
...
function writeRuntimeProbeFixture() {
  const dir = join(projectRoot, ".tmp", "task251-fixtures");
...
function writeAgentRunAdapterFixture() {
  const dir = join(projectRoot, ".tmp", "task251-fixtures");
  mkdirSync(dir, { recursive: true });
  const path = join(dir, "attempt-review-agent-run-adapter.mjs");
  writeFileSync(
    path,
    [
      "process.stdout.write('task251 stdout before terminal attempt review gate\\n');",
      "process.stderr.write('task251 stderr before terminal attempt review gate\\n');",
      "process.stdout.write('# Agent Report\\n\\n## Input Context\\nTask251 attempt review fixture.\\n\\n## Claims Proposed\\nproof_authority: none\\n\\n## Evidence Produced\\nRuntime logs only.\\n\\n## Graph Patch\\nNo trusted mutation.\\n\\n## Blockers\\nNo terminal unattended completion.\\n');",
      ""
    ].join("\n"),
    "utf8"
  );
...
async function createServiceOwnedAgentRunRoute(suffix) {
  ...
  const workstream = spawnWorkstream(projectRoot, {
    ...
    kind: "proof_route",
    goal: `Task251 service-owned terminal attempt review fixture ${suffix}.`,
    created_by: "goal3-task251"
  });
  const agentRun = await executeProfileAgentRun(projectRoot, {
    ...
    goal: `Run Task251 terminal attempt review fixture ${suffix}.`,
    context_path: `.comath/workstreams/${workstream.workstream_id}/spec.yaml`,
    actor: `goal3-task251-agent-run-${suffix}`
  });
...
function idsFor(suffix) {
  return {
    ...
    attemptReviewId: `LIFE-UNATTENDED-EXEC-ATTEMPT-REVIEW-${suffix}`
  };
}
...
function orchestrationInput(ids, route, runtimeFixture) {
  return {
    ...
    actor: `goal3-task251-orchestrator ${projectRoot}\\secret.txt Authorization: Bearer plain-token proof_success durable transport provided long-lived websocket`,
    runtime_probe: {
      probe_id: ids.probeId,
      pi_host_label: `pi-host-task251-${ids.suffix}`,
    ...
    operator_session: {
      ...
      last_result_summary: {
        summary: "automatic checkpoint chain ready for terminal attempt review gate",
    ...
    transport_recovery: {
      ...
      reconnect_reason: "automatic real-Pi lifecycle checkpoint recovery before terminal attempt review"
    },
    transport_lease: {
      ...
      open_reason: "bounded lease before terminal attempt review"
    },
    transport_heartbeat: {
      ...
      heartbeat_reason: "bounded heartbeat before terminal attempt review"
    },
    guided_execution: {
...
function preparedReviewInput(ids, orchestration, continuity) {
  return {
    ...
    actor: `goal3-task251-review ${projectRoot}\\secret.txt Authorization: Bearer plain-token proof_success durable transport provided long-lived websocket handoff can execute`,
...
function approvalInput(ids, handoffReview) {
  return {
    ...
    actor: `goal3-task251-approval ${projectRoot}\\secret.txt Authorization: Bearer plain-token proof_success durable transport provided live transport open operator approval recorded`,
    handoff_review_id: ids.handoffReviewId,
...
function executorContractInput(ids, handoffReview) {
  return {
    ...
    actor: `goal3-task251-executor ${projectRoot}\\secret.txt Authorization: Bearer plain-token proof_success durable transport provided live transport open production unattended executor`,
    handoff_review_id: ids.handoffReviewId,
    handoff_review_path: handoffReview.handoff_review_path,
    handoff_review_sha256: handoffReview.handoff_review_artifact.sha256,
...
function durableTransportContractInput(ids, handoffReview, approval, executorContract) {
  ...
  return {
    ...
    actor: `goal3-task251-durable-transport ${projectRoot}\\secret.txt Authorization: Bearer plain-token proof_success durable transport provided live transport open`,
    handoff_review_id: ids.handoffReviewId,
    handoff_review_path: handoffReview.handoff_review_path,
    handoff_review_sha256: handoffReview.handoff_review_artifact.sha256,
...
function readinessInput(ids, handoffReview, approval, executorContract, durableTransportContract) {
  return {
    ...
    actor: `goal3-task251-readiness ${projectRoot}\\secret.txt Authorization: Bearer plain-token proof_success durable transport provided live transport open operator approval recorded`,
    handoff_review_id: ids.handoffReviewId,
    handoff_review_path: handoffReview.handoff_review_path,
    handoff_review_sha256: handoffReview.handoff_review_artifact.sha256,
    requested_execution_mode: "production_unattended_real_host",
...
function executionAttemptInput(ids, readiness, overrides = {}) {
  return {
    ...
    actor: `goal3-task251-attempt ${projectRoot}\\secret.txt Authorization: Bearer plain-token proof_success durable transport provided live transport open unattended real-host execution completed`,
    readiness_id: ids.readinessId,
...
function attemptReviewInput(ids, attempt, overrides = {}) {
  return {
    project_id: projectId,
    attempt_review_id: ids.attemptReviewId,
    actor: `goal3-task251-attempt-review ${projectRoot}\\secret.txt Authorization: Bearer plain-token proof_success durable transport provided live transport open unattended real-host execution completed`,
    attempt_id: ids.attemptId,
    attempt_path: attempt.attempt_path,
    attempt_sha256: attempt.attempt_artifact.sha256,
    ...overrides
  };
}

const init = initProject({ name: "Goal 3 Task251 terminal execution attempt review", root_path: projectRoot });
...
async function createReadyChain(suffix) {
  ...
  const continuity = recordPiCodexLifecycleOperatorServiceTransportContinuity(projectRoot, {
    ...
    actor: `goal3-task251-continuity ${projectRoot}\\secret.txt Authorization: Bearer plain-token durable transport provided`,
    transport_contract_id: ids.contractId,
    ...
    client_epoch: 11,
    continuity_reason: "prepared handoff continuity before terminal attempt review gate"
  });
...
try {
  ...
  assert.equal(
    typeof reviewPiCodexLifecycleUnattendedRealHostExecutionAttempt,
    "function",
    "Task251 must export a service-owned terminal attempt review gate"
  );
  assert.equal(
    getComathdStatus().capabilities.includes("pi_codex_unattended_real_host_execution_attempt_review_gate"),
    true,
    "Task251 capability ledger must advertise the terminal attempt review gate"
  );

  const { ids, readiness } = await createReadyChain("0251");
  const blockedAttempt = recordPiCodexLifecycleUnattendedRealHostExecutionAttempt(
    projectRoot,
    executionAttemptInput(ids, readiness)
  );
  const blockedReview = reviewPiCodexLifecycleUnattendedRealHostExecutionAttempt(
    projectRoot,
    attemptReviewInput(ids, blockedAttempt)
  );
  assert.equal(blockedReview.schema_version, "comath.pi_codex_unattended_real_host_execution_attempt_review.v1");
  assert.equal(blockedReview.attempt_review_status, "blocked_unattended_real_host_executor_unavailable");
  assert.equal(blockedReview.terminal_goal_state, "blocked_with_replayable_certificate");
  assert.deepEqual(blockedReview.blocker_reasons, ["service_owned_unattended_executor_unavailable"]);
  assert.equal(blockedReview.attempt_id, ids.attemptId);
  assert.equal(blockedReview.attempt_status, "blocked_unattended_real_host_executor_unavailable");
  assert.equal(blockedReview.attempt_artifact.sha256, blockedAttempt.attempt_artifact.sha256);
  assert.equal(blockedReview.attempt_current, true);
  assert.equal(blockedReview.readiness_current, true);
  assert.equal(blockedReview.service_owned_attempt_review_completed, true);
  assert.equal(blockedReview.terminal_unattended_completion_certified, false);
  assert.equal(blockedReview.execution_attempted, false);
  assert.equal(blockedReview.execution_attempt_succeeded, false);
  assert.equal(blockedReview.unattended_real_host_execution_completed, false);
  ...
  assert.doesNotMatch(JSON.stringify(blockedReview), privilegedPublicTerms);
  assert.doesNotMatch(JSON.stringify(blockedReview), transportOverclaimTerms);
  assert.doesNotMatch(JSON.stringify(blockedReview), unattendedOverclaimTerms);

  const persistedReviewPath = join(projectRoot, blockedReview.attempt_review_path);
  assert.equal(existsSync(persistedReviewPath), true, "attempt review must persist append-only evidence");
  const persistedReview = readJson(persistedReviewPath);
  assert.equal(persistedReview.attempt_review_artifact, undefined, "persisted review must not self-hash recursively");
  assert.equal(persistedReview.proof_authority, "none");
  assert.equal(persistedReview.can_certify_ga, false);
  assert.doesNotMatch(JSON.stringify(persistedReview), secretTerms);
  assert.doesNotMatch(JSON.stringify(persistedReview), privilegedPublicTerms);
  assert.doesNotMatch(JSON.stringify(persistedReview), transportOverclaimTerms);
  assert.doesNotMatch(JSON.stringify(persistedReview), unattendedOverclaimTerms);
  ...
  assert.throws(
    ...
    "attempt reviews must be append-only by review id"
  );
  assert.throws(
    () =>
      reviewPiCodexLifecycleUnattendedRealHostExecutionAttempt(
        projectRoot,
        attemptReviewInput(ids, blockedAttempt, {
          attempt_review_id: "LIFE-UNATTENDED-EXEC-ATTEMPT-REVIEW-0251-STALE",
          attempt_sha256: "d".repeat(64)
        })
      ),
    { code: "PI_CODEX_UNATTENDED_REAL_HOST_EXECUTION_ATTEMPT_REVIEW_ATTEMPT_STALE" },
    "attempt review must reject stale attempt hashes"
  );

  const attemptPath = join(projectRoot, blockedAttempt.attempt_path);
  const attemptBeforePoison = readFileSync(attemptPath, "utf8");
  const poisonedAttempt = readJson(attemptPath);
  poisonedAttempt.proof_authority = "lean_kernel_clean_replay";
  poisonedAttempt.can_certify_ga = true;
  poisonedAttempt.unattended_real_host_execution_completed = true;
  writeJson(attemptPath, poisonedAttempt);
  assert.throws(
    () =>
      reviewPiCodexLifecycleUnattendedRealHostExecutionAttempt(
        projectRoot,
        attemptReviewInput(ids, blockedAttempt, {
          attempt_review_id: "LIFE-UNATTENDED-EXEC-ATTEMPT-REVIEW-0251-POISONED",
          attempt_sha256: sha256Text(readFileSync(attemptPath, "utf8"))
        })
      ),
    { code: "PI_CODEX_UNATTENDED_REAL_HOST_EXECUTION_ATTEMPT_REVIEW_ATTEMPT_INVALID" },
    "attempt review must reject promotional attempt artifacts even when the hash is current"
  );
  writeFileSync(attemptPath, attemptBeforePoison, "utf8");

  const executorChain = await createReadyChain("0251-RUN");
  ...
  const attemptedReview = reviewPiCodexLifecycleUnattendedRealHostExecutionAttempt(
    projectRoot,
    attemptReviewInput(executorChain.ids, attempted)
  );
  assert.equal(attemptedReview.attempt_review_status, "blocked_terminal_unattended_completion_review_required");
  assert.equal(attemptedReview.terminal_goal_state, "blocked_with_replayable_certificate");
  assert.deepEqual(attemptedReview.blocker_reasons, ["terminal_unattended_completion_evidence_missing"]);
  assert.equal(attemptedReview.execution_attempted, true);
  assert.equal(attemptedReview.execution_attempt_succeeded, true);
  assert.equal(attemptedReview.execution_attempt_result_artifact_current, true);
  assert.equal(attemptedReview.terminal_unattended_completion_certified, false);
  assert.equal(attemptedReview.unattended_real_host_execution_completed, false);
  assert.equal(attemptedReview.proof_authority, "none");
  assert.equal(attemptedReview.can_certify_ga, false);

  const failedChain = await createReadyChain("0251-FAIL");
  ...
  const failedReview = reviewPiCodexLifecycleUnattendedRealHostExecutionAttempt(
    projectRoot,
    attemptReviewInput(failedChain.ids, failedAttempt)
  );
  assert.equal(failedReview.attempt_review_status, "blocked_unattended_real_host_execution_attempt_failed");
  assert.deepEqual(failedReview.blocker_reasons, ["service_owned_unattended_executor_failed"]);
  assert.equal(failedReview.execution_attempted, true);
  assert.equal(failedReview.execution_attempt_succeeded, false);
  assert.equal(failedReview.terminal_unattended_completion_certified, false);
  assert.equal(failedReview.unattended_real_host_execution_completed, false);
  assert.equal(failedReview.proof_authority, "none");
  assert.equal(failedReview.can_certify_ga, false);

  const server = createComathServer();
  const routeChain = await createReadyChain("0251-ROUTE");
  const routeAttempt = recordPiCodexLifecycleUnattendedRealHostExecutionAttempt(
    projectRoot,
    executionAttemptInput(routeChain.ids, routeChain.readiness)
  );
  const routeResponse = await server.inject({
    method: "POST",
    path: "/release/pi-codex-lifecycle/unattended-real-host-execution-attempt-review",
    body: attemptReviewInput(routeChain.ids, routeAttempt, {
      project_root: projectRoot,
      actor: "goal3-task251-route token=plain-token proof_success durable transport provided live transport open unattended real-host execution completed"
    })
  });
  assert.equal(routeResponse.status, 200, JSON.stringify(routeResponse.body));
  assert.equal(routeResponse.body.execution_attempt_review.proof_authority, "none");
  assert.equal(routeResponse.body.execution_attempt_review.terminal_goal_state, "blocked_with_replayable_certificate");
  assert.equal(routeResponse.body.execution_attempt_review.unattended_real_host_execution_completed, false);
  assert.equal(routeResponse.body.execution_attempt_review.can_certify_ga, false);
  ...
  const blockedAuditEvent = events.find(
    (event) =>
      event.event_type === "release.pi_codex_unattended_real_host_execution_attempt_reviewed" &&
      event.payload.attempt_review_id === ids.attemptReviewId
  );
  assert.ok(blockedAuditEvent, "blocked attempt review audit event must be appended");
  assert.equal(blockedAuditEvent.payload.attempt_id, ids.attemptId);
  assert.equal(blockedAuditEvent.payload.attempt_artifact_sha256, blockedAttempt.attempt_artifact.sha256);
  assert.equal(blockedAuditEvent.payload.attempt_review_artifact_sha256, blockedReview.attempt_review_artifact.sha256);
  assert.equal(blockedAuditEvent.payload.terminal_goal_state, "blocked_with_replayable_certificate");
  assert.equal(blockedAuditEvent.payload.terminal_unattended_completion_certified, false);
  ...
  const attemptedAuditEvent = events.find(
    (event) =>
      event.event_type === "release.pi_codex_unattended_real_host_execution_attempt_reviewed" &&
      event.payload.attempt_review_id === executorChain.ids.attemptReviewId
  );
  assert.ok(attemptedAuditEvent, "recorded attempt review audit event must be appended");
  assert.equal(attemptedAuditEvent.payload.attempt_review_status, "blocked_terminal_unattended_completion_review_required");
  assert.equal(attemptedAuditEvent.payload.execution_attempt_succeeded, true);
  assert.equal(attemptedAuditEvent.payload.terminal_unattended_completion_certified, false);
...
console.log("Goal 3 Task251 terminal execution attempt review tests passed.");
