...
const selected = manifest.tasks.filter((task) => task.task_id >= "PM-090" && task.task_id <= "PM-100");
assert.equal(selected.length, 11, "PM-090 through PM-100 must form the final 11-task bounded tranche");
...
for (const taskId of ["PM-091", "PM-092", "PM-093", "PM-094", "PM-095", "PM-096", "PM-097", "PM-098", "PM-099", "PM-100"]) {
  assert.equal(byTask.get(taskId).category, "tactic repair", `${taskId} must stay in the tactic repair segment`);
}

const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task60-tranche-"));
...
assert.equal(trancheReport.start_task_id, "PM-090");
assert.equal(trancheReport.end_task_id, "PM-100");
...
assert.deepEqual(trancheReport.category_counts, {
  "theorem-search-assisted": 1,
  "tactic repair": 10
});
assert.deepEqual(trancheReport.non_authority_input_counts, {
  "theorem-search-assisted": { task_count: 1, proof_authority: "none" },
  "tactic repair": { task_count: 10, proof_authority: "none" }
});
assert.deepEqual(trancheReport.no_reinvent_guards, {
...
assert.equal(trancheReport.tranche_report_path, ".comath/release/positive_matrix/PM-090_PM-100/lean_authority_executor_tranche_v1.json");
assert.equal(replayCommands.length, 22, "each PM-090 through PM-100 task should attempt check and build through the generic executor");
...
assert.deepEqual(persistedTrancheReport.non_authority_input_counts, trancheReport.non_authority_input_counts);
assert.deepEqual(persistedTrancheReport.no_reinvent_guards, trancheReport.no_reinvent_guards);
...
for (const result of trancheReport.results) {
  assert.match(result.task_id, /^PM-09[0-9]$|^PM-100$/);
...
assert.equal(
  existsSync(join(projectRoot, ".comath/release/positive_matrix/PM-089/lean_authority_executor_blocker_v3.json")),
  false,
  "Task 60 tranche must not write previous-batch PM-089 executor artifacts"
);
assert.equal(
  existsSync(join(projectRoot, ".comath/release/positive_matrix/PM-101/lean_authority_executor_blocker_v3.json")),
  false,
  "Task 60 tranche must not write out-of-range PM-101 executor artifacts"
);

console.log("Goal 3 Task 60 PM-090 through PM-100 generic Lean executor test passed.");
