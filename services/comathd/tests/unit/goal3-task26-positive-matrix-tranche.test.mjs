...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task26-positive-tranche-"));
...
assert.equal(tranche.tranche_scope.start_task_id, "PM-035");
assert.equal(tranche.tranche_scope.end_task_id, "PM-045");
...
assert.deepEqual(
  ...
  ["PM-035", "PM-036", "PM-037", "PM-038", "PM-039", "PM-040", "PM-041", "PM-042", "PM-043", "PM-044", "PM-045"]
);
assert.equal(tranche.summary.clean_replay_passed, 0, "Task 26 must not fake clean replay for PM-035 through PM-045");
assert.equal(tranche.summary.replayable_blocker, 11);
assert.equal(tranche.summary.promoted_count, 0);
assert.equal(tranche.next_batch_scope.start_after_task_id, "PM-045");
assert.equal(tranche.next_batch_scope.remaining_tasks, 55);

const byTask = new Map(tranche.results.map((result) => [result.task_id, result]));
for (const taskId of ["PM-035", "PM-036", "PM-037", "PM-038", "PM-039", "PM-040"]) {
  assert.equal(byTask.get(taskId).category, "real analysis", `${taskId} must stay in the real-analysis tranche segment`);
}
for (const taskId of ["PM-041", "PM-042", "PM-043", "PM-044", "PM-045"]) {
...
assert.equal(
  existsSync(join(projectRoot, ".comath/release/positive_matrix/PM-046/replay_attempt_certificate.json")),
  false,
  "Task 26 tranche must not write the next-batch PM-046 certificate"
);

console.log("Goal 3 Task 26 positive matrix tranche tests passed.");
