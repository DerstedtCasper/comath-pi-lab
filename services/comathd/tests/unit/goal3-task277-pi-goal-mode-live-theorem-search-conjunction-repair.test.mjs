...
const root = mkdtempSync(join(tmpdir(), "comath-goal3-task277-live-theorem-search-conjunction-repair-"));
const liveApiKey = "loogle_live_secret_should_not_be_persisted_277";
...
function writeFixtures(projectRoot) {
  ...
  writeFileSync(
    ...
    [
      "# Source",
      "",
      "Theorem. Live theorem-search declaration hints may guide a conjunction repair, but LeanRunner remains the checker.",
      "Proof sketch. Loogle returns `And.intro`; CoMath may use it as candidate context only.",
      ""
    ].join("\n")
  );
  writeFileSync(join(projectRoot, "lean", "Main.lean"), "theorem goal3_task277_source : True /\\ True := by exact And.intro trivial trivial\n");
}
...
function installFakeLeanLakeCommands(projectRoot, mode, leanVersion = "9.99.9") {
  ...
  const previousLog = process.env.COMATH_TASK277_RUN_LOG;
  process.env.COMATH_TASK277_RUN_LOG = logPath;
    ...
    const lakeBody =
      mode === "reject"
        ? [
            ...
            "  exit /b 0",
            ")",
            "echo %CD%^|%*>>\"%COMATH_TASK277_RUN_LOG%\"",
            "echo tactic failed, theorem-search And.intro hint may repair conjunction 1>&2",
            "exit /b 1",
            ""
          ]
        : [
            ...
            "findstr /C:\"exact And.intro trivial trivial\" LeanCandidate.lean >nul || exit /b 6",
            "exit /b 0",
            ""
          ];
    ...
    const lakeBody =
      mode === "reject"
        ? [
            ...
            "  exit 0",
            "fi",
            "printf '%s|%s\\n' \"$PWD\" \"$*\" >> \"$COMATH_TASK277_RUN_LOG\"",
            "echo 'tactic failed, theorem-search And.intro hint may repair conjunction' >&2",
            "exit 1",
            ""
          ]
        : [
            ...
            "grep -q 'exact And.intro trivial trivial' LeanCandidate.lean || exit 6",
            "exit 0",
            ""
          ];
  ...
  return {
    ...
    restore: () => {
      ...
      if (previousLog === undefined) {
        delete process.env.COMATH_TASK277_RUN_LOG;
      } else {
        process.env.COMATH_TASK277_RUN_LOG = previousLog;
      }
    }
  };
}

function startLiveLoogleFixtureServer() {
  const requests = [];
        ...
        results: [
          {
            ...
            declaration_type: "a -> b -> a /\\ b",
            module: "Init.Core",
            import_hint: "Init",
            source_url: "https://loogle.lean-lang.org/",
            mathlib_revision: "fixture-revision",
            score: 0.98
          }
        ]
      })
...
async function tickUntil(campaignId, predicate, label, maxTicks = 18) {
  ...
  for (let index = 0; index < maxTicks; index += 1) {
    body = await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task277" });
...
const previousEnv = {
  ...
  COMATH_LOOGLE_SEARCH_BASE_URL: process.env.COMATH_LOOGLE_SEARCH_BASE_URL,
  COMATH_LOOGLE_API_KEY: process.env.COMATH_LOOGLE_API_KEY
};
...
try {
  writeFixtures(root);
  liveServer = await startLiveLoogleFixtureServer();
  process.env.COMATH_ENABLE_GOAL_MODE_LIVE_REPAIR_HINT_EXECUTION = "1";
  process.env.COMATH_LIVE_REPAIR_HINT_PROVIDERS = "theorem_search:loogle";
  process.env.COMATH_LOOGLE_SEARCH_BASE_URL = liveServer.baseUrl;
  process.env.COMATH_LOOGLE_API_KEY = liveApiKey;

  const started = startCampaign({
    project_root: root,
    project_name: "Goal 3 Task 277 Live Theorem Search Conjunction Repair",
    user_goal:
      "Prove theorem goal3_task277 : True /\\ True := by sorry while preserving the exact theorem boundary and using Loogle And.intro only as candidate repair context.",
  ...
  for (const candidate of candidates) {
    ...
    writeFileSync(
      join(root, leanRel),
      [
        "-- intentionally wrong but placeholder-free candidate; LeanRunner must reject it before live theorem-search repairs it.",
        "theorem goal3_task277 : True /\\ True := by",
        "  exact True.intro",
        ""
      ].join("\n")
    );
  ...
  const resumed = await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task277" });
  ...
  const liveResult = hintExecution.adapter_results.find(
    (result) => result.kind === "theorem_search" && result.provider === "loogle"
  );
  ...
  assert.equal(liveResult.result_payload_summary.results[0].declaration_name, "And.intro");
  assert.equal(liveResult.result_payload_summary.results[0].proof_authority, "none");

  const repaired = await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task277" });
  ...
  for (const item of repaired.repair_execution.per_candidate_executions) {
    ...
    assert.equal(item.placeholder_free_repair_strategy, "live_theorem_search_and_intro_conjunction_repair");
    ...
    assert.match(stripLineComments(repairedText), /theorem goal3_task277\s*:\s*True\s*\/\\\s*True\s*:=\s*by\s+exact And\.intro trivial trivial/u);
  }
...
console.log("Goal 3 Task 277 Pi goal-mode live theorem-search conjunction repair test passed.");
