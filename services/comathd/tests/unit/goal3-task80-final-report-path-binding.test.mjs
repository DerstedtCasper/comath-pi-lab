...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task80-report-path-binding-"));
...
try {
  const { project } = initProject({ name: "Goal 3 Task 80 Report Path Binding", root_path: projectRoot });
  const taskId = "PM-080";
  const claimStatement = "theorem Goal3Positive080 : True";
  ...
  const replayId = "RPLY-0080";
  const runId = "LRUN-0080";
  ...
  const target = writeProjectFile(
    ...
    [
      ...
      "",
      "theorem Goal3Positive080 : True := by",
      "  trivial",
      "",
      "#check Goal3Positive080",
      "#print axioms Goal3Positive080",
      "",
      "end MathResearch",
      ""
    ].join("\n")
  );
  const audit = writeProjectFile(`${cleanRootRel}/Audit/TargetAudit.lean`, "import MathResearch.Target\n#check MathResearch.Goal3Positive080\n#print axioms MathResearch.Goal3Positive080\n");
  ...
  const formalSpec = writeProjectFile(formalSpecRel, `${JSON.stringify({
    ...
    claim_id: claim.id,
    namespace: "MathResearch",
    theorem_name: "Goal3Positive080",
    theorem_header: "theorem Goal3Positive080 : True",
  ...
  const stdout = writeProjectFile(`.comath/evidence/${claim.id}/lean/${runId}.stdout.log`, "Goal3Positive080 checked\n");
  ...
  const alternateStructuredAuditRel = `.comath/evidence/${claim.id}/lean/alternate_structured_audit.json`;
  const alternateDependencyClosureRel = `.comath/evidence/${claim.id}/lean/alternate_dependency_closure.json`;
  const alternateAxiomProfileRel = `.comath/evidence/${claim.id}/lean/alternate_axiom_profile.json`;
  const alternateStatementCheckRel = `.comath/evidence/${claim.id}/lean/alternate_statement_equivalence.json`;
  ...
  writeProjectFile(alternateStructuredAuditRel, `${JSON.stringify({ result: "pass", hard_vetoes: [], generated_by_run_id: "foreign-run" }, null, 2)}\n`);
  writeProjectFile(alternateDependencyClosureRel, `${JSON.stringify({ result: "pass", hard_vetoes: [], source: "foreign" })}\n`);
  writeProjectFile(alternateAxiomProfileRel, `${JSON.stringify({ result: "pass", detected_axioms: [], hard_vetoes: [], source: "foreign" })}\n`);
  writeProjectFile(alternateStatementCheckRel, `${JSON.stringify({ result: "pass", locked_statement_hash: claim.statement_hash, hard_vetoes: [], source: "foreign" })}\n`);
  ...
  const finalReplayManifest = createFinalReplayManifestV3({
    ...
    campaign_id: "CAM-0080",
    claim_id: claim.id,
    theorem_name: "MathResearch.Goal3Positive080",
  ...
  const report = packageGoal3GaPositiveMatrixFinalAuthorityEvidenceWithDerivedBindingsV3({
    ...
    evidence: {
      ...
      structured_audit_path: alternateStructuredAuditRel,
      dependency_closure_path: alternateDependencyClosureRel,
      axiom_profile_path: alternateAxiomProfileRel,
      statement_check_path: alternateStatementCheckRel,
  ...
  const evidence = appendEvidenceRecord(projectRoot, {
    ...
    claim_id: claim.id,
    kind: "lean",
    summary: "Final authority report paths must be bound to the FinalReplayManifest report_paths.",
  ...
  assert.equal(rejected.gate.ok, false, "foreign pass reports must not satisfy final-authority promotion");
  assert.ok(rejected.gate.vetoes.includes("final authority derived binding manifest mismatch"));
} finally {
  rmSync(projectRoot, { recursive: true, force: true });
}

console.log("Goal 3 Task 80 final report path binding test passed.");
