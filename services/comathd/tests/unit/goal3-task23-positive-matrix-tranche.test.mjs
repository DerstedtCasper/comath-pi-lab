...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task23-positive-tranche-"));
...
assert.equal(tranche.tranche_scope.start_task_id, "PM-013");
assert.equal(tranche.tranche_scope.end_task_id, "PM-023");
...
assert.deepEqual(
  ...
  ["PM-013", "PM-014", "PM-015", "PM-016", "PM-017", "PM-018", "PM-019", "PM-020", "PM-021", "PM-022", "PM-023"]
);
assert.equal(tranche.summary.clean_replay_passed, 0, "Task 23 must not fake clean replay for PM-013 through PM-023");
assert.equal(tranche.summary.replayable_blocker, 11);
assert.equal(tranche.summary.promoted_count, 0);
assert.equal(tranche.next_batch_scope.start_after_task_id, "PM-023");
assert.equal(tranche.next_batch_scope.remaining_tasks, 77);

const byTask = new Map(tranche.results.map((result) => [result.task_id, result]));
for (const taskId of ["PM-013", "PM-014", "PM-015", "PM-016", "PM-017", "PM-018", "PM-019", "PM-020"]) {
  assert.equal(byTask.get(taskId).category, "algebra", `${taskId} must stay in the algebra tranche segment`);
}
for (const taskId of ["PM-021", "PM-022", "PM-023"]) {
...
assert.equal(
  existsSync(join(projectRoot, ".comath/release/positive_matrix/PM-024/replay_attempt_certificate.json")),
  false,
  "Task 23 tranche must not write the next-batch PM-024 certificate"
);

console.log("Goal 3 Task 23 positive matrix tranche tests passed.");
