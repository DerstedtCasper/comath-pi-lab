...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task89-real-replay-env-"));
...
try {
  const { project } = initProject({ name: "Goal 3 Task 89 Real Replay Environment Diagnostic", root_path: projectRoot });
  ...
  const injectedProbeFinalAuthority = executeGoal3GaPositiveMatrixLeanAuthorityReplay({
    projectRoot,
    taskId: "PM-084",
    claimId: claim.id,
    materialSource,
    completeFinalAuthorityEvidence: true,
    probeLeanVersion: () => ({ exit_code: 0, stdout: "Lean (version 4.23.0, x86_64-unknown, commit injected)", stderr: "" }),
    probeLakeVersion: () => ({ exit_code: 0, stdout: "Lake version 5.0.0 injected", stderr: "" })
  });
  assert.equal(injectedProbeFinalAuthority.executor_status, "blocked_before_replay");
  assert.equal(injectedProbeFinalAuthority.blocker_code, "lean_authority_evidence_incomplete");
  assert.match(injectedProbeFinalAuthority.blocker_detail, /Injected version probes cannot produce final Lean Authority evidence/);
  assert.deepEqual(injectedProbeFinalAuthority.final_authority_packaging.lean_run_manifest_paths, []);
  assert.equal(injectedProbeFinalAuthority.final_authority_packaging.proof_authority, "none");
  ...
  const archived = await archiveGoal3GaPositiveMatrixRealReplayAttemptEvidence({
    ...
    actor: "goal3-task89",
    probeLeanVersion: () => ({ exit_code: 1, stdout: "", stderr: "error: no default toolchain configured" }),
    probeLakeVersion: () => ({ exit_code: 1, stdout: "", stderr: "error: no default toolchain configured" })
  });
  ...
  assert.equal(archived.environment_diagnostic.diagnostic_is_proof_authority, false);
  assert.equal(archived.environment_diagnostic.can_run_clean_replay, false);
  assert.equal(archived.environment_diagnostic.blocker_code, "lean_toolchain_unavailable_for_live_replay");
  assert.deepEqual(archived.environment_diagnostic.probed_commands, [
    ["lean", "--version"],
    ["lake", "--version"]
  ]);
  assert.equal(archived.environment_diagnostic.lean_version_probe.exit_code, 1);
  assert.match(archived.environment_diagnostic.lean_version_probe.stderr_excerpt, /no default toolchain/);
  assert.equal(archived.environment_diagnostic.lake_version_probe.exit_code, 1);
  assert.match(archived.environment_diagnostic.lake_version_probe.stderr_excerpt, /no default toolchain/);
  ...
  assert.deepEqual(archiveOnDisk.environment_diagnostic, archived.environment_diagnostic);

  const finalReplayMaterial = JSON.parse(readFileSync(join(projectRoot, materialSource.final_replay_manifest_v3_path), "utf8"));
  assert.equal(finalReplayMaterial.result, "blocked");
...
console.log("Goal 3 Task 89 real replay environment diagnostic test passed.");
