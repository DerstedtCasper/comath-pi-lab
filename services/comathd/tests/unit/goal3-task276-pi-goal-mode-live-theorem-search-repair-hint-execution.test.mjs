...
const root = mkdtempSync(join(tmpdir(), "comath-goal3-task276-live-theorem-search-hint-"));
const liveApiKey = "loogle_live_secret_should_not_be_persisted_276";
...
function writeFixtures(projectRoot) {
  ...
  writeFileSync(
    ...
    [
      "# Source",
      "",
      "Theorem. Failed Lean diagnostics should trigger service-owned live theorem-search hints when explicitly enabled.",
      "Proof sketch. Loogle results are repair context only and never proof authority.",
      ""
    ].join("\n")
  );
  ...
  if (process.platform === "win32") {
    ...
    writeFileSync(
      ...
      [
        ...
        "echo tactic failed, theorem-search hints may help repair 1>&2",
        "exit /b 1",
        ""
      ].join("\r\n")
    );
    ...
    writeFileSync(
      ...
      [
        ...
        "echo 'tactic failed, theorem-search hints may help repair' >&2",
        "exit 1",
        ""
      ].join("\n")
    );
...
async function advanceTo(campaignId, targetStage) {
  ...
  for (let index = 0; index < 14; index += 1) {
    body = await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task276" });
...
try {
  ...
  const started = startCampaign({
    project_root: root,
    project_name: "Goal 3 Task 276 Live Theorem Search Repair Hint Execution",
    user_goal:
      "Prove theorem goal3_task276 : True := by sorry while preserving the exact theorem boundary and using Loogle only as repair context.",
  ...
  await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task276" });
  await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task276" });
  await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task276" });
  await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task276" });
  await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task276" });
  ...
  const resumed = await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task276" });
  ...
  assert.equal(liveServer.requests[0].method, "GET");
  assert.equal(liveServer.requests[0].authorization, `Bearer ${liveApiKey}`);
  assert.equal(liveServer.requests[0].limit, "3");
  assert.match(liveServer.requests[0].query, /locked_statement_hash=/u);
  ...
  assert.equal(liveResult.result_payload_summary.result_kind, "theorem_search_results");
  assert.equal(liveResult.result_payload_summary.live_provider.provider, "loogle");
  ...
  assert.equal(liveResult.result_payload_summary.results.length, 1);
  assert.equal(liveResult.result_payload_summary.results[0].provider, "loogle");
  ...
  assert.equal(liveResult.result_payload_summary.results[0].can_promote_claim, false);
  ...
  const repaired = await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task276" });
...
console.log("Goal 3 Task 276 Pi goal-mode live theorem-search repair hint execution test passed.");
