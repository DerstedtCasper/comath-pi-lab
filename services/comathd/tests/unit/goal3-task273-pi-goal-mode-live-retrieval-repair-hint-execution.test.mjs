...
const root = mkdtempSync(join(tmpdir(), "comath-goal3-task273-live-retrieval-hint-"));
const liveApiKey = "jina_live_secret_should_not_be_persisted_273";
...
function writeFixtures(projectRoot) {
  ...
  writeFileSync(
    ...
    [
      "# Source",
      "",
      "Theorem. Failed Lean diagnostics should trigger service-owned live retrieval repair hints when explicitly enabled.",
      "Proof sketch. Retrieved text is quoted evidence only and never proof authority.",
      ""
    ].join("\n")
  );
  ...
  if (process.platform === "win32") {
    ...
    writeFileSync(
      ...
      [
        ...
        "echo tactic failed, live literature hints may help repair 1>&2",
        "exit /b 1",
        ""
      ].join("\r\n")
    );
    ...
    writeFileSync(
      ...
      [
        ...
        "echo 'tactic failed, live literature hints may help repair' >&2",
        "exit 1",
        ""
      ].join("\n")
    );
  ...
  const server = createServer((request, response) => {
    ...
    response.end(
      [
        ...
        "",
        "Result: a retrieved repair hint for the locked statement.",
        "This document is quoted evidence only and does not certify the theorem.",
        ""
      ].join("\n")
    );
  });
...
async function advanceTo(campaignId, targetStage) {
  ...
  for (let index = 0; index < 14; index += 1) {
    body = await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task273" });
...
try {
  ...
  const started = startCampaign({
    project_root: root,
    project_name: "Goal 3 Task 273 Live Retrieval Repair Hint Execution",
    user_goal:
      "Prove theorem goal3_task273 : True := by sorry while preserving the exact theorem boundary and using live retrieval only as repair context.",
  ...
  await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task273" });
  await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task273" });
  await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task273" });
  await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task273" });
  await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task273" });
  ...
  const resumed = await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task273" });
  ...
  assert.equal(liveResult.result_payload_summary.result_kind, "retrieval_results");
  assert.equal(liveResult.result_payload_summary.live_provider.provider, "jina_search");
  ...
  assert.equal(liveResult.result_payload_summary.results[0].provider, "jina_search");
  ...
  assert.equal(liveResult.result_payload_summary.results[0].prompt_injection_scan.status, "pass");
  ...
  const repaired = await tickCampaign({ project_root: root, campaign_id: campaignId, actor: "goal3-task273" });
...
console.log("Goal 3 Task 273 Pi goal-mode live retrieval repair hint execution test passed.");
