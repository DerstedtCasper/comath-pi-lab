...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task28-positive-tranche-"));
...
assert.equal(tranche.tranche_scope.start_task_id, "PM-046");
assert.equal(tranche.tranche_scope.end_task_id, "PM-056");
...
assert.deepEqual(
  ...
  ["PM-046", "PM-047", "PM-048", "PM-049", "PM-050", "PM-051", "PM-052", "PM-053", "PM-054", "PM-055", "PM-056"]
);
assert.equal(tranche.summary.clean_replay_passed, 0, "Task 28 must not fake clean replay for PM-046 through PM-056");
assert.equal(tranche.summary.replayable_blocker, 11);
assert.equal(tranche.summary.promoted_count, 0);
assert.equal(tranche.next_batch_scope.start_after_task_id, "PM-056");
assert.equal(tranche.next_batch_scope.remaining_tasks, 44);

const byTask = new Map(tranche.results.map((result) => [result.task_id, result]));
for (const taskId of ["PM-046", "PM-047", "PM-048", "PM-049", "PM-050"]) {
  assert.equal(byTask.get(taskId).category, "topology", `${taskId} must stay in the topology tranche segment`);
}
for (const taskId of ["PM-051", "PM-052", "PM-053", "PM-054", "PM-055", "PM-056"]) {
  assert.equal(byTask.get(taskId).category, "combinatorics", `${taskId} must stay in the combinatorics tranche segment`);
}
...
for (const result of tranche.results) {
  ...
  assert.ok(result.blockers.includes("requires_live_mathlib_or_domain_formalization"));
...
assert.equal(
  existsSync(join(projectRoot, ".comath/release/positive_matrix/PM-057/replay_attempt_certificate.json")),
  false,
  "Task 28 tranche must not write the next-batch PM-057 certificate"
);

console.log("Goal 3 Task 28 positive matrix tranche tests passed.");
