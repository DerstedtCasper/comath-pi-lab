...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task75-theorem-identity-"));
...
try {
  const { project } = initProject({ name: "Goal 3 Task 75 Theorem Identity Binding", root_path: projectRoot });
  const taskId = "PM-075";
  const claimStatement = "theorem Goal3Positive075 : True";
  ...
  const replayId = "RPLY-0075";
  const runId = "LRUN-0075";
  ...
  const target = writeProjectFile(
    ...
    [
      ...
      "",
      "theorem Goal3Positive075 : True := by",
      "  trivial",
      "",
      "#check Goal3Positive075",
      "#print axioms Goal3Positive075",
      "",
      "end MathResearch",
      ""
    ].join("\n")
  );
  const audit = writeProjectFile(`${cleanRootRel}/Audit/TargetAudit.lean`, "import MathResearch.Target\n#check MathResearch.Goal3Positive075\n#print axioms MathResearch.Goal3Positive075\n");
  ...
  const formalSpec = writeProjectFile(formalSpecRel, `${JSON.stringify({
    ...
    task_id: taskId,
    claim_id: claim.id,
    theorem_name: "Goal3Positive075",
    theorem_header: "def Goal3Foreign075 : True",
  ...
  const stdout = writeProjectFile(`.comath/evidence/${claim.id}/lean/${runId}.stdout.log`, "Goal3Positive075 checked\n");
  ...
  const finalReplayManifest = createFinalReplayManifestV3({
    ...
    campaign_id: "CAM-0075",
    claim_id: claim.id,
    theorem_name: "MathResearch.Goal3Positive075",
  ...
  const evidence = appendEvidenceRecord(projectRoot, {
    ...
    claim_id: claim.id,
    kind: "lean",
    summary: "Same-claim and same-statement theorem-header identity drift must fail closed.",
  ...
  assert.equal(rejected.gate.ok, false, "same-statement theorem-header identity drift must not satisfy the ordinary promotion gate");
} finally {
  rmSync(projectRoot, { recursive: true, force: true });
}

console.log("Goal 3 Task 75 derived binding theorem identity test passed.");
