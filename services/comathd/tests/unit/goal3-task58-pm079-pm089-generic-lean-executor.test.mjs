...
const selected = manifest.tasks.filter((task) => task.task_id >= "PM-079" && task.task_id <= "PM-089");
assert.equal(selected.length, 11, "PM-079 through PM-089 must form an 11-task bounded tranche");

const byTask = new Map(selected.map((task) => [task.task_id, task]));
for (const taskId of ["PM-079", "PM-080"]) {
  assert.equal(byTask.get(taskId).category, "paper-to-formal-spec", `${taskId} must stay in the paper-to-formal-spec segment`);
}
for (const taskId of ["PM-081", "PM-082", "PM-083", "PM-084", "PM-085", "PM-086", "PM-087", "PM-088", "PM-089"]) {
  assert.equal(byTask.get(taskId).category, "theorem-search-assisted", `${taskId} must stay in the theorem-search-assisted segment`);
}

const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task58-tranche-"));
...
assert.equal(trancheReport.start_task_id, "PM-079");
assert.equal(trancheReport.end_task_id, "PM-089");
...
assert.deepEqual(trancheReport.category_counts, {
  "paper-to-formal-spec": 2,
  "theorem-search-assisted": 9
});
assert.deepEqual(trancheReport.non_authority_input_counts, {
  "paper-to-formal-spec": { task_count: 2, proof_authority: "none" },
  "theorem-search-assisted": { task_count: 9, proof_authority: "none" }
});
...
assert.equal(trancheReport.tranche_report_path, ".comath/release/positive_matrix/PM-079_PM-089/lean_authority_executor_tranche_v1.json");
assert.equal(replayCommands.length, 22, "each PM-079 through PM-089 task should attempt check and build through the generic executor");
...
for (const result of trancheReport.results) {
  assert.match(result.task_id, /^PM-07[9]$|^PM-08[0-9]$/);
...
assert.equal(
  existsSync(join(projectRoot, ".comath/release/positive_matrix/PM-090/lean_authority_executor_blocker_v3.json")),
  false,
  "Task 58 tranche must not write next-batch PM-090 executor artifacts"
);

console.log("Goal 3 Task 58 PM-079 through PM-089 generic Lean executor test passed.");
