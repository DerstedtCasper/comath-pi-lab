...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task25-positive-tranche-"));
...
assert.equal(tranche.tranche_scope.start_task_id, "PM-024");
assert.equal(tranche.tranche_scope.end_task_id, "PM-034");
...
assert.deepEqual(
  ...
  ["PM-024", "PM-025", "PM-026", "PM-027", "PM-028", "PM-029", "PM-030", "PM-031", "PM-032", "PM-033", "PM-034"]
);
assert.equal(tranche.summary.clean_replay_passed, 0, "Task 25 must not fake clean replay for PM-024 through PM-034");
assert.equal(tranche.summary.replayable_blocker, 11);
assert.equal(tranche.summary.promoted_count, 0);
assert.equal(tranche.next_batch_scope.start_after_task_id, "PM-034");
assert.equal(tranche.next_batch_scope.remaining_tasks, 66);

const byTask = new Map(tranche.results.map((result) => [result.task_id, result]));
for (const taskId of ["PM-024", "PM-025", "PM-026", "PM-027", "PM-028", "PM-029", "PM-030"]) {
  assert.equal(byTask.get(taskId).category, "order", `${taskId} must stay in the order tranche segment`);
}
for (const taskId of ["PM-031", "PM-032", "PM-033", "PM-034"]) {
...
assert.equal(
  existsSync(join(projectRoot, ".comath/release/positive_matrix/PM-035/replay_attempt_certificate.json")),
  false,
  "Task 25 tranche must not write the next-batch PM-035 certificate"
);

console.log("Goal 3 Task 25 positive matrix tranche tests passed.");
