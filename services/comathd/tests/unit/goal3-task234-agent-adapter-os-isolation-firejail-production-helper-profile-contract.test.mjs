...
function createFirejailHelperScript(projectRoot) {
  const helperScript = join(projectRoot, "task234-firejail-provider-helper.mjs");
  writeFileSync(
    helperScript,
    [
...
function assertFirejailProfileContract(contract, source, helperSha256, message) {
  assert.ok(contract, `${message}: production helper profile contract must be present`);
  assert.equal(contract.contract_source, "service_owned_firejail_production_helper_profile_contract");
  assert.equal(contract.provider, "firejail");
  assert.equal(contract.provider_family, "firejail");
  ...
  assert.deepEqual(contract.required_host_facility_tools, ["firejail_cli"]);
  assert.equal(contract.runtime_family, "firejail_private_netnone_nonewprivs");
  ...
  assert.equal(contract.firejail_profile_or_policy_inspection_allowed, false);
  assert.equal(contract.firejail_command_execution_required_for_profile_contract, false);
...
const helperEnvVar = "COMATH_AGENT_ADAPTER_OSISO_FIREJAIL_HELPER";
const helperArgsEnvVar = "COMATH_AGENT_ADAPTER_OSISO_FIREJAIL_HELPER_ARGS_JSON";
...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task234-firejail-production-helper-profile-"));
...
try {
  assert.equal(
    ...
    true,
    "Task234 capability ledger must advertise the Firejail production-helper profile contract without claiming GA"
  );

  const focusedTestName = "goal3-task234-agent-adapter-os-isolation-firejail-production-helper-profile-contract.test.mjs";
  ...
  for (const [path, pattern] of [
    ["README.md", /Task234.*Firejail.*production-helper profile contract/s],
    ["AGENTS.md", /Task234.*Firejail.*production-helper profile contract/s],
    ["TODO.md", /Task213-234 summary/s],
    ["REVIEW.md", /Goal 3 Task 234/s],
    ["config/README.md", /Task234.*Firejail.*production-helper profile contract/s],
    ["docs/architecture/adapter-contracts.md", /Task234.*Firejail.*production-helper profile contract/s],
    ["docs/architecture/threat-model.md", /Task234.*Firejail.*production-helper profile contract/s]
  ]) {
    assert.match(readRepoFile(path), pattern, `${path} must document Task234 without overclaiming`);
  }
  ...
  const providerConfig = sampleConfig?.agentAdapterOsIsolation?.providerHelpers?.find(
    (entry) => entry?.provider === "firejail"
  );
  assert.equal(providerConfig?.productionHelperProfileContractRequired, true);
  assert.equal(providerConfig?.productionHelperProfileContractKind, "firejail_private_netnone_nonewprivs_profile_contract");
  assert.deepEqual(providerConfig?.productionHelperProfileSources, [
    "operator_configured_provider_helper",
    "bundled_provider_helper_protocol_asset"
  ]);
  assert.deepEqual(providerConfig?.requiredHostFacilityTools, ["firejail_cli"]);

  const helperScript = createFirejailHelperScript(projectRoot);
  ...
  const init = initProject({ name: "Goal3 Task234 Firejail Production Helper Profile", root_path: projectRoot });
  const projectId = init.project.project_id;
  const launch = prepareAgentAdapterOsIsolationSandboxLaunch(projectRoot, {
    project_id: projectId,
    launch_id: "ADAPTER-OSISO-LAUNCH-0234-FIREJAIL",
    ...
    launcher_probe: () => ({
      ...
      launcher_version: "firejail-launcher-task234",
  ...
  const runner = prepareAgentAdapterOsIsolationProviderRunner(projectRoot, {
    project_id: projectId,
    runner_id: "ADAPTER-OSISO-RUNNER-0234-FIREJAIL",
    ...
    runner_environment: {
      ...
      firejail_profile_path: `${projectRoot}/etc/firejail/profile`
    }
  });
  assert.equal(runner.ok, true);
  assert.equal(runner.provider, "firejail");
  ...
  assert.equal(JSON.stringify(runner).includes("firejail/profile"), false);

  const hostCapability = probeAgentAdapterOsIsolationProviderHostCapability(projectRoot, {
    project_id: projectId,
    host_capability_probe_id: "ADAPTER-OSISO-HOST-CAP-0234-FIREJAIL",
    adapter_id: "codex-cli",
    backend: "external",
    ...
    provider_host_capability_probe: () => ({
      ...
      capability_facts: ["task234 firejail host capability observed"],
      required_tools: [{ name: "firejail_cli", present: true, version: null, binary_sha256: helperSha256 }],
      kernel_features: ["task234-firejail-host-capability"],
      diagnostics: [`${projectRoot} host capability diagnostic must be scrubbed`, "Firejail capability observed"]
    })
  });
  const firejailSupportedHost = process.platform === "linux";
  assert.equal(hostCapability.ok, firejailSupportedHost);
  ...
  assert.equal(
    hostCapability.provider_host_capability.required_tools.some((tool) => tool.name === "firejail_cli"),
    true
  );

  if (!firejailSupportedHost) {
    ...
    const hostValidator = ({ host_validation_id }) => ({
      ...
      helper_version: "firejail-helper-task234",
      ...
      self_test_transcript_sha256: sha256Text(`task234:${host_validation_id}`),
      self_test_args_prefix_sha256: sha256Text("task234-firejail-prefix"),
      self_test_args_prefix_count: 1,
      self_test_fixed_args_sha256: sha256Text("task234-firejail-fixed-args"),
      diagnostics: [`${projectRoot} host validator diagnostic must be scrubbed`, "Firejail helper host validated"]
    });

    const host = validateAgentAdapterOsIsolationProviderHelperHost(projectRoot, {
      project_id: projectId,
      host_validation_id: "ADAPTER-OSISO-HELPER-HOST-0234-FIREJAIL",
    ...
    const driftExecution = runAgentAdapterOsIsolationProviderHelperExecution(projectRoot, {
      project_id: projectId,
      helper_execution_id: "ADAPTER-OSISO-HELPER-0234-FIREJAIL-DRIFT",
    ...
    assert.equal(
      driftExecution.ok,
      false,
      "helper execution must fail closed if the Firejail helper profile source drifts after host validation"
    );
    ...
    const helperExecution = runAgentAdapterOsIsolationProviderHelperExecution(projectRoot, {
      project_id: projectId,
      helper_execution_id: "ADAPTER-OSISO-HELPER-0234-FIREJAIL",
      host_validation_id: host.host_validation_id,
      runner_id: runner.runner_id,
      launch_id: launch.launch_id,
      adapter_id: "codex-cli",
    ...
    const collection = collectAgentAdapterOsIsolationProviderHelperExecutionEvidence(projectRoot, {
      project_id: projectId,
      collection_id: "ADAPTER-OSISO-HELPER-COLLECT-0234-FIREJAIL",
...
console.log("Goal 3 Task234 Firejail production-helper profile contract tests passed.");
