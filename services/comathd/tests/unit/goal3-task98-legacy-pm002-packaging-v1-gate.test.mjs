...
const projectRoot = mkdtempSync(join(tmpdir(), "comath-goal3-task98-legacy-pm002-v1-"));
...
try {
  const { project } = initProject({ name: "Goal 3 Task 98 Legacy PM-002 V1 Gate", root_path: projectRoot });
  ...
  const claimStatement = "theorem Goal3Positive098 (M : Type) [Monoid M] (x : M) : 1 * x = x";
  ...
  const finalRunManifestRel = `.comath/evidence/${claim.id}/lean/LRUN-0098.manifest.json`;
  ...
  appendFinalReplayRegistryEntryV3(projectRoot, finalReplayManifest, { project_id: project.project_id, actor: "goal3-task98" });
  ...
  const packaging = packageGoal3GaPm002FinalAuthorityEvidence({
    ...
    materialSource: {
      ...
      lean_run_manifest_id: "LRUN-0098",
  ...
  const leanEvidence = appendEvidenceRecord(projectRoot, {
    ...
    claim_id: claim.id,
    kind: "lean",
    summary: "Legacy PM-002 v1 packaging is intentionally non-authoritative for formal promotion.",
  ...
  assert.equal(promoted.gate.ok, false, "legacy PM-002 v1 packaging must not be promotion-grade final authority evidence");
  assert.equal(promoted.claim.status, "conjectural");
  assert.ok(promoted.gate.vetoes.includes("formally_checked requires Lean Authority v3 final replay packaging"));
} finally {
  rmSync(projectRoot, { recursive: true, force: true });
}

console.log("Goal 3 Task 98 legacy PM-002 packaging v1 gate test passed.");
