...
function spansPath(projectRoot: string): string {
  return paperPath(projectRoot, "spans.json");
}
...
function parsePaperSpans(value: unknown): PaperSpan[] {
  return z.array(paperSpanSchema).parse(value);
}
...
export function initPaper(projectRoot: string, input: InitPaperInput): PaperState {
  ...
  if (!existsSync(spansPath(projectRoot))) {
    writeJson(spansPath(projectRoot), []);
  }
...
export function readPaperState(projectRoot: string, projectId: string): PaperState {
  ...
  return {
    ...
    margin_notes: notes,
    spans: readPaperSpans(projectRoot, projectId),
    sections
  };
}
...
export function readPaperSpans(projectRoot: string, projectId?: string): PaperSpan[] {
  const spans = readJson(spansPath(projectRoot), [], parsePaperSpans);
  return projectId ? spans.filter((span) => span.project_id === projectId) : spans;
}

export function writePaperSpans(projectRoot: string, spans: PaperSpan[]): PaperSpan[] {
  const parsed = z.array(paperSpanSchema).parse(spans);
  writeJson(spansPath(projectRoot), parsed);
  return parsed;
}
...
export function renderClaimBlock(projectRoot: string, input: RenderClaimBlockInput): RenderedClaimBlock {
  ...
  if (vetoes.length > 0) {
    appendAuditEvent(projectRoot, {
      project_id: input.project_id,
      event_type: "paper.claim_block_rejected",
      actor: input.actor,
      target_id: claim.id,
      payload: {
        wording: input.wording,
        ...
        proposed_margin_note_id: note.id
      }
    });
    return {
      markdown,
  ...
  const existingSpans = readPaperSpans(projectRoot);
  const span = paperSpanSchema.parse({
    id: nextSequentialId("PS", existingSpans.map((item) => item.id)),
    project_id: input.project_id,
    paper_id: manifest.paper_id,
    section_id: "__rendered_claim_blocks__",
    claim_id: claim.id,
    text_sha256: note.rendered_block_sha256,
    margin_note_ids: [note.id],
    evidence_ids: note.evidence_ids,
    workstream_ids: sourceWorkstreams,
    status: blockers.length ? "blocked" : "draft",
    created_at: timestamp,
    updated_at: timestamp
  });
  writePaperSpans(projectRoot, [...existingSpans, span]);
  appendProvenanceEvent(projectRoot, {
    project_id: input.project_id,
    event_type: "paper.span_recorded",
    actor: input.actor,
    target_id: claim.id,
    payload: {
      span_id: span.id,
      margin_note_ids: span.margin_note_ids,
      evidence_ids: span.evidence_ids,
      workstream_ids: span.workstream_ids,
...
export function checkPaper(projectRoot: string, input: { project_id: string }): PaperCheckResult {
  ...
  const spans = readPaperSpans(projectRoot, input.project_id);
  const claimRefs = collectClaimRefs(markdown);
  const spanEvents = readProvenanceEvents(projectRoot, input.project_id).filter(
    (event) => event.event_type === "paper.span_recorded"
  );

  for (const span of spans) {
    const spanTextAppears = claimRefs.some((ref) => sha256Text(ref.block.trim()) === span.text_sha256);
    if (!spanTextAppears) {
      vetoes.push("orphan_paper_span");
    }
    const spanEvent = spanEvents.find(
      (event) =>
        event.payload.span_id === span.id &&
        event.payload.text_sha256 === span.text_sha256 &&
        Array.isArray(event.payload.margin_note_ids) &&
        span.margin_note_ids.every((id) => (event.payload.margin_note_ids as unknown[]).includes(id))
    );
    if (!spanEvent) {
      vetoes.push("missing_span_provenance");
    }
    for (const marginNoteId of span.margin_note_ids) {
      if (!marginRead.notes.some((note) => note.id === marginNoteId && note.project_id === input.project_id)) {
        vetoes.push("missing_span_margin_note");
      }
    }
  }
  ...
  for (const ref of claimRefs) {
    ...
    const span = spans.find(
      (item) =>
        ...
        item.margin_note_ids.includes(marginNote.id)
    );
    if (!span) {
      vetoes.push("missing_paper_span");
    } else {
      for (const marginNoteId of span.margin_note_ids) {
        if (!marginRead.notes.some((note) => note.id === marginNoteId && note.project_id === input.project_id)) {
          vetoes.push("missing_span_margin_note");
