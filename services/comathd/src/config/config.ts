...
export const researchConfigSchema = z.strictObject({
  ...
  proof_workflow: z.strictObject({
    ...
    tool_budget: taskBudgetSchema.refine(value => value.output_tokens === 0 && value.tool_calls >= 8 && value.token_enforcement !== "wall_only_legacy",
      "Proof tools need zero LLM tokens and an explicit budget for version/check/build/audit/lock-elaboration/dependency calls"),
