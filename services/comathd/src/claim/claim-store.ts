...
function sameClaimMutationVersion(left: Claim, right: Claim): boolean {
  return (
    ...
    left.gate_result_id === right.gate_result_id &&
    left.dependency_closure_status === right.dependency_closure_status &&
    left.formalization_status === right.formalization_status &&
    left.audit_state === right.audit_state &&
...
export function updateClaim(projectRoot: string, input: UpdateClaimInput): Claim {
  ...
  const nextAssumptions = input.patch.assumptions ?? previous.assumptions;
  const nextDomain = input.patch.domain ?? previous.domain;
  const semanticChanged =
    nextStatement !== previous.statement ||
    JSON.stringify(nextAssumptions) !== JSON.stringify(previous.assumptions) ||
    nextDomain !== previous.domain;
  const updated = claimSchema.parse({
    ...
    domain: nextDomain,
    status: semanticChanged ? input.patch.status ?? "conjectural" : input.patch.status ?? previous.status,
    evidence_level: semanticChanged ? 0 : input.patch.evidence_level ?? previous.evidence_level,
    gate_result_id: semanticChanged ? undefined : previous.gate_result_id,
    dependency_closure_status: semanticChanged ? "unchecked" : previous.dependency_closure_status,
    formalization_status: semanticChanged ? "none" : previous.formalization_status,
    audit_state: semanticChanged ? "not_audited" : previous.audit_state,
    updated_at: now()
  });
...
export function applyGatePromotedClaim(projectRoot: string, claim: Claim, expectedClaim?: Claim): Claim {
  const claims = readClaims(projectRoot);
  const index = claims.findIndex((item) => item.project_id === claim.project_id && item.id === claim.id);
  if (index === -1) {
    throw new ComathError("claim not found", { statusCode: 404, code: "CLAIM_NOT_FOUND" });
  }
  if (expectedClaim && !sameClaimMutationVersion(claims[index], expectedClaim)) {
    throw new ComathError("claim changed during mutation", { statusCode: 409, code: "CLAIM_CHANGED_DURING_MUTATION" });
  }
