...
import { appendProvenanceEvent } from "../provenance/ledger.js";
...
export async function runMathProveBridgeMock(
  ...
  const predictedArtifactId = nextSequentialId(
    "AR",
    listArtifactRefs(projectRoot).map((artifact) => artifact.id)
  );
  const predictedEvidenceId = nextSequentialId(
    "EV",
    readEvidenceRecords(projectRoot).map((evidence) => evidence.id)
  );
  const manifest = mathProveRunManifestSchema.parse({
    id: report.id,
    project_id: request.project_id,
    claim_id: request.claim_id,
    mode: request.mode,
    run_root: join(".comath", "evidence", request.claim_id, "mathprove"),
    status: result.ok ? "approved" : "failed",
    target_status: request.target_status,
    status_json_artifact_id: predictedArtifactId,
    final_audit_artifact_id: request.mode === "final_audit" ? predictedArtifactId : undefined,
    evidence_ids: [predictedEvidenceId],
    artifact_ids: [predictedArtifactId],
    ...
    `${JSON.stringify(
      {
        ...
        bridge: "mathprove",
        result,
        mathprove_run_manifest: manifest
      },
      null,
      2
    )}\n`,
  ...
  appendProvenanceEvent(projectRoot, {
    project_id: request.project_id,
    event_type: "mathprove.run_recorded",
    actor: request.actor,
    target_id: request.claim_id,
    payload: {
      mode: request.mode,
