...
import {
  hasFormalProofAuditCertification,
  hasFormalProofDependencyCertification,
  hasFormalizationCertification,
  hasTrustedFormalProofRun
} from "./formal-proof.js";
...
function appendGateResult(projectRoot: string, gate: GateResult): void {
  const path = gateResultsPath(projectRoot);
  mkdirSync(dirname(path), { recursive: true });
  appendFileSync(path, `${JSON.stringify(gate)}\n`, "utf8");
}
...
function vetoesForRequest(projectRoot: string, claim: Claim, request: ClaimPromotionRequest): string[] {
  ...
  if (request.target_status === "formally_checked") {
    ...
    if (!hasFormalizationCertification(projectRoot, request.project_id, request.claim_id, request.artifact_ids)) {
      vetoes.push("formally_checked requires formalization certification");
    }
    ...
    if (!hasFormalProofDependencyCertification(projectRoot, request.project_id, request.claim_id, request.artifact_ids)) {
      vetoes.push("formally_checked requires dependency closure certification");
    }
    if (claim.audit_state !== "audit_passed") {
      vetoes.push("formally_checked requires audit_passed");
    }
    if (!hasFormalProofAuditCertification(projectRoot, request.project_id, request.claim_id, request.artifact_ids)) {
      vetoes.push("formally_checked requires formal proof audit certification");
    }
    if (!hasTrustedFormalProofRun(projectRoot, request.project_id, request.claim_id, request.artifact_ids)) {
      vetoes.push("formally_checked requires kernel_checked formal proof run");
    }
  }
...
function runClaimPromotionGateForClaim(projectRoot: string, claim: Claim, request: ClaimPromotionRequest): GateResult {
...
export function runClaimPromotionGate(projectRoot: string, request: ClaimPromotionRequest): GateResult {
  ...
  return runClaimPromotionGateForClaim(projectRoot, claim, request);
}

function applyClaimPromotionDecision(
  projectRoot: string,
  claim: Claim,
  gate: GateResult,
  actor: string
  ...
  const stored = applyGatePromotedClaim(projectRoot, promoted, claim);
  appendAuditEvent(projectRoot, {
    project_id: gate.project_id,
...
export function promoteClaim(projectRoot: string, request: ClaimPromotionRequest): ClaimPromotionDecision {
  ...
  const gate = runClaimPromotionGateForClaim(projectRoot, claim, request);
  appendGateResult(projectRoot, gate);
  return {
    gate,
