...
public static class CoMathOwnedJob {
 ...
 static void Emit(string json) { lock(outputLock) { Console.WriteLine(json); Console.Out.Flush(); } }
 ...
 public static int Run(string program,string[] args,string cwd,string environment,string nonce,int timeout) {
  IntPtr job=IntPtr.Zero,outRead=IntPtr.Zero,outWrite=IntPtr.Zero,errRead=IntPtr.Zero,errWrite=IntPtr.Zero,input=IntPtr.Zero,env=IntPtr.Zero,attributes=IntPtr.Zero,handles=IntPtr.Zero;
  PI process=new PI(); bool assigned=false; Task stdout=null,stderr=null;
  try {
   ...
   input=CreateFile("NUL",0x80000000,3,ref security,3,0,IntPtr.Zero);Check(input!=new IntPtr(-1),"stdin_nul");
   ...
   Task.Run(delegate {try{while(Console.ReadLine()!=null){cancelled=true;break;}}finally{cancelled=true;}});
   ...
   while(true){ACCOUNT account;Check(QueryInformationJobObject(job,1,out account,(uint)Marshal.SizeOf(typeof(ACCOUNT)),IntPtr.Zero),"query_job");if(account.active==0)break;
    if(!terminated&&(cancelled||watch.ElapsedMilliseconds>=timeout)){timedOut=!cancelled;Check(TerminateJobObject(job,1),"terminate_job");terminated=true;stopAt=watch.ElapsedMilliseconds;}
    if(terminated&&watch.ElapsedMilliseconds-stopAt>10000)throw new InvalidOperationException("termination_unconfirmed");Thread.Sleep(10);
   }
   Task.WaitAll(new[]{stdout,stderr});uint code;Check(GetExitCodeProcess(process.process,out code),"exit_code");
   Emit("{\"type\":\"completed\",\"exit_code\":"+code+",\"timed_out\":"+(timedOut?"true":"false")+",\"cancelled\":"+(cancelled?"true":"false")+",\"termination_confirmed\":true}");return 0;
...
$exitCode = [CoMathOwnedJob]::Run([string]$request.program, [string[]]@($request.args), [string]$request.cwd, $environmentBlock, [string]$request.nonce, [int]$request.timeout_ms)
exit $exitCode
