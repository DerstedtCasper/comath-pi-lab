...
export function authenticateWorker(runtime: ProjectRuntime, authorization: string | undefined, checkpointOnly = false): WorkerPrincipal {
  ...
  if (!task || task.generation !== Number(attempt.generation) || attempt.fenced_at || Number(attempt.termination_confirmed) !== 0
    || !Number.isFinite(Date.parse(String(attempt.expires_at))) || now >= Date.parse(String(attempt.expires_at)) || (!grace && task.status !== "running")) {
