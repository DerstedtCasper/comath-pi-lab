...
import {
  ...
  type Goal3ReleaseCandidateProofBreadthSelectedTrancheNextClosurePackagingResultsFollowUp,
  type Goal3ReleaseCandidateProofBreadthSelectedTrancheNextClosurePackagingResultsFollowUpInput
} from "./goal3-proof-breadth-selected-tranche-next-closure-packaging-results-follow-up.js";
import type { Goal3ReleaseCandidateProofBreadthSelectedTrancheNextClosurePackagingResultsClosureExecutionPackagingFollowThrough } from "./goal3-proof-breadth-selected-tranche-next-closure-packaging-results-closure-execution-packaging-follow-through.js";
...
type PersistedTask352ClosureExecutionPackagingFollowThrough = Omit<
...
export type Goal3ReleaseCandidateProofBreadthSelectedTrancheNextClosurePackagingResultsClosureExecutionPackagingResultsFollowUpInput = {
  project_id: string;
  ...
  selected_tranche_next_closure_packaging_results_closure_execution_packaging_follow_through_sha256: string;
  actor?: string;
  requested_follow_up_mode?: "open_formal_workbench_release_candidate_selected_tranche_next_closure_packaging_results_closure_execution_packaging_results_follow_up";
};

export type Goal3ReleaseCandidateProofBreadthSelectedTrancheNextClosurePackagingResultsClosureExecutionPackagingResultsFollowUp = {
  schema_version: "comath.goal3_release_candidate_proof_breadth_selected_tranche_next_closure_packaging_results_closure_execution_packaging_results_follow_up.v1";
  selected_tranche_next_closure_packaging_results_closure_execution_packaging_results_follow_up_id: string;
  project_id: string;
  actor: string;
  created_at: string;
  ok: boolean;
  follow_up_status:
    | "next_closure_execution_selected_tranche_packaging_results_current_for_proof_breadth_closure_recheck"
...
function task352Path(followThroughId: string): string {
  return normalizeRelativePath(
    join(
      ".comath",
      "release",
...
function assertSafeId(value: string | undefined, label: string): string {
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{2,180}$/u.test(id)) {
    throw new ComathError(`${label} is invalid`, {
      statusCode: 400,
      code: "GOAL3_SELECTED_TRANCHE_NEXT_CLOSURE_PACKAGING_RESULTS_CLOSURE_EXECUTION_PACKAGING_RESULTS_FOLLOW_UP_INVALID_ID"
    });
  }
  return id;
}
...
function assertSha256(value: string | undefined): string {
  ...
  if (!/^[a-f0-9]{64}$/u.test(sha256)) {
    throw new ComathError("Goal 3 selected-tranche next closure execution packaging results follow-up hash is invalid", {
      statusCode: 400,
      code: "GOAL3_SELECTED_TRANCHE_NEXT_CLOSURE_PACKAGING_RESULTS_CLOSURE_EXECUTION_PACKAGING_RESULTS_FOLLOW_UP_INVALID_HASH"
    });
  }
  return sha256;
}

function assertTask352InputPath(path: string | undefined, followThroughId: string): string {
  const normalizedInputPath = normalizeRelativePath(typeof path === "string" ? path.trim() : "");
  const expectedPath = task352Path(followThroughId);
  if (normalizedInputPath !== expectedPath) {
    throw new ComathError("Goal 3 selected-tranche next closure execution packaging results follow-up Task352 path mismatch", {
      statusCode: 400,
      code: "GOAL3_SELECTED_TRANCHE_NEXT_CLOSURE_PACKAGING_RESULTS_CLOSURE_EXECUTION_PACKAGING_RESULTS_FOLLOW_UP_TASK352_PATH_MISMATCH"
    });
  }
  return expectedPath;
}
...
function sanitizeActor(actor: string | undefined): string {
  const text =
    typeof actor === "string" && actor.trim().length > 0
      ? actor.trim()
      : "goal3-selected-tranche-next-closure-execution-packaging-results-follow-up";
  return text
...
function assertCurrentArtifactReference(
  ...
  try {
    ...
    throw new ComathError("Goal 3 selected-tranche next closure execution packaging results follow-up Task352 artifact is invalid", {
      statusCode: 400,
...
function parseTask352(bytes: Buffer): PersistedTask352ClosureExecutionPackagingFollowThrough {
  try {
    return JSON.parse(bytes.toString("utf8")) as PersistedTask352ClosureExecutionPackagingFollowThrough;
  } catch {
    throw new ComathError("Goal 3 selected-tranche next closure execution packaging results follow-up Task352 artifact is invalid", {
      statusCode: 400,
      code: "GOAL3_SELECTED_TRANCHE_NEXT_CLOSURE_PACKAGING_RESULTS_CLOSURE_EXECUTION_PACKAGING_RESULTS_FOLLOW_UP_INVALID_TASK352"
...
function readTask352Artifact(
  projectRoot: string,
  projectId: string,
  followThroughId: string,
  followThroughPath: string,
  expectedSha256: string
): { body: PersistedTask352ClosureExecutionPackagingFollowThrough; artifact: ArtifactReference } {
  const absolutePath = assertPathAllowed(projectRoot, followThroughPath, { purpose: "read", resolveRealpath: true });
  if (!existsSync(absolutePath) || !statSync(absolutePath).isFile()) {
    throw new ComathError("Goal 3 selected-tranche next closure execution packaging results follow-up Task352 artifact is missing", {
      statusCode: 404,
      code: "GOAL3_SELECTED_TRANCHE_NEXT_CLOSURE_PACKAGING_RESULTS_CLOSURE_EXECUTION_PACKAGING_RESULTS_FOLLOW_UP_TASK352_MISSING"
    });
  }
  const bytes = readFileSync(absolutePath);
  const actualSha256 = sha256Bytes(bytes);
  if (actualSha256 !== expectedSha256) {
    throw new ComathError("Goal 3 selected-tranche next closure execution packaging results follow-up Task352 artifact is stale", {
      statusCode: 409,
      code: "GOAL3_SELECTED_TRANCHE_NEXT_CLOSURE_PACKAGING_RESULTS_CLOSURE_EXECUTION_PACKAGING_RESULTS_FOLLOW_UP_STALE_TASK352"
    });
  }
  const body = parseTask352(bytes);
  if (
    body.schema_version !==
    ...
    body.selected_tranche_next_closure_packaging_results_closure_execution_packaging_follow_through_id !==
      followThroughId ||
    normalizeRelativePath(body.selected_tranche_next_closure_packaging_results_closure_execution_packaging_follow_through_path) !==
      followThroughPath ||
    ...
    typeof body.selected_tranche_next_closure_packaging_results_closure_execution_bridge_id !== "string" ||
    typeof body.selected_tranche_next_closure_packaging_results_closure_execution_bridge_path !== "string" ||
    !isArtifactReference(body.selected_tranche_next_closure_packaging_results_closure_execution_bridge_artifact) ||
    typeof body.selected_tranche_next_closure_packaging_follow_through_id !== "string" ||
    typeof body.selected_tranche_next_closure_packaging_follow_through_path !== "string" ||
    !isArtifactReference(body.selected_tranche_next_closure_packaging_follow_through_artifact)
  ) {
    throw new ComathError("Goal 3 selected-tranche next closure execution packaging results follow-up Task352 artifact is invalid", {
      statusCode: 400,
      code: "GOAL3_SELECTED_TRANCHE_NEXT_CLOSURE_PACKAGING_RESULTS_CLOSURE_EXECUTION_PACKAGING_RESULTS_FOLLOW_UP_INVALID_TASK352"
  ...
  assertCurrentArtifactReference(
    projectRoot,
...
function callerProofOrGaMaterialPresent(
  input: Goal3ReleaseCandidateProofBreadthSelectedTrancheNextClosurePackagingResultsClosureExecutionPackagingResultsFollowUpInput
): boolean {
...
function artifactForDelegatedTask347(
  projectRoot: string,
  delegated: Goal3ReleaseCandidateProofBreadthSelectedTrancheNextClosurePackagingResultsFollowUp
): ArtifactReference {
  return artifactForProjectFile(
    projectRoot,
    delegated.selected_tranche_next_closure_packaging_results_follow_up_path,
    "goal3_release_candidate_proof_breadth_selected_tranche_next_closure_packaging_results_follow_up"
  );
}

export function recordGoal3ReleaseCandidateProofBreadthSelectedTrancheNextClosurePackagingResultsClosureExecutionPackagingResultsFollowUp(
  projectRoot: string,
  input: Goal3ReleaseCandidateProofBreadthSelectedTrancheNextClosurePackagingResultsClosureExecutionPackagingResultsFollowUpInput
  ...
  const task352Sha256 = assertSha256(
    input.selected_tranche_next_closure_packaging_results_closure_execution_packaging_follow_through_sha256
  );
  const task352RelativePath = assertTask352InputPath(
    input.selected_tranche_next_closure_packaging_results_closure_execution_packaging_follow_through_path,
    task352Id
  );
  const requestedMode =
  ...
  if (
    ...
    throw new ComathError("Goal 3 selected-tranche next closure execution packaging results follow-up mode is invalid", {
      statusCode: 400,
      code: "GOAL3_SELECTED_TRANCHE_NEXT_CLOSURE_PACKAGING_RESULTS_CLOSURE_EXECUTION_PACKAGING_RESULTS_FOLLOW_UP_INVALID_MODE"
    });
  }
  ...
  if (existsSync(absoluteOutputPath)) {
    throw new ComathError("Goal 3 selected-tranche next closure execution packaging results follow-up already exists", {
      statusCode: 409,
      code: "GOAL3_SELECTED_TRANCHE_NEXT_CLOSURE_PACKAGING_RESULTS_CLOSURE_EXECUTION_PACKAGING_RESULTS_FOLLOW_UP_ALREADY_EXISTS"
    });
  }

  const task352 = readTask352Artifact(projectRoot, projectId, task352Id, task352RelativePath, task352Sha256);
  const actor = sanitizeActor(input.actor);
  const delegatedInput = {
    project_id: projectId,
    selected_tranche_next_closure_packaging_results_follow_up_id: `${followUpId}-TASK347`,
    selected_tranche_next_closure_packaging_follow_through_id:
      task352.body.selected_tranche_next_closure_packaging_follow_through_id,
    selected_tranche_next_closure_packaging_follow_through_path:
      task352.body.selected_tranche_next_closure_packaging_follow_through_path,
    selected_tranche_next_closure_packaging_follow_through_sha256:
      task352.body.selected_tranche_next_closure_packaging_follow_through_artifact.sha256,
    actor,
  ...
  } satisfies Goal3ReleaseCandidateProofBreadthSelectedTrancheNextClosurePackagingResultsFollowUpInput;
  const delegated = recordGoal3ReleaseCandidateProofBreadthSelectedTrancheNextClosurePackagingResultsFollowUp(
    projectRoot,
    delegatedInput
  );
  const delegatedArtifact = artifactForDelegatedTask347(projectRoot, delegated);
  const callerAuthorityMaterialIgnored = callerProofOrGaMaterialPresent(input);
  const blockerReasons = uniqueStrings([
    ...task352.body.blocker_reasons,
  ...
  const body = {
    ...
    selected_tranche_next_closure_packaging_results_closure_execution_packaging_results_follow_up_id: followUpId,
    project_id: projectId,
    actor,
    ...
    follow_up_status: delegated.ok
      ? "next_closure_execution_selected_tranche_packaging_results_current_for_proof_breadth_closure_recheck"
      : "blocked_next_closure_execution_selected_tranche_packaging_results_incomplete",
    selected_tranche_next_closure_packaging_results_closure_execution_packaging_results_follow_up_path: outputPath,
    requested_follow_up_mode:
      "open_formal_workbench_release_candidate_selected_tranche_next_closure_packaging_results_closure_execution_packaging_results_follow_up",
    selected_tranche_next_closure_packaging_results_closure_execution_packaging_follow_through_id: task352Id,
    ...
    selected_tranche_next_closure_packaging_results_closure_execution_bridge_id:
      task352.body.selected_tranche_next_closure_packaging_results_closure_execution_bridge_id,
    selected_tranche_next_closure_packaging_results_closure_execution_bridge_path:
      task352.body.selected_tranche_next_closure_packaging_results_closure_execution_bridge_path,
    selected_tranche_next_closure_packaging_results_closure_execution_bridge_artifact:
      task352.body.selected_tranche_next_closure_packaging_results_closure_execution_bridge_artifact,
    selected_tranche_next_closure_packaging_follow_through_id:
      task352.body.selected_tranche_next_closure_packaging_follow_through_id,
    selected_tranche_next_closure_packaging_follow_through_path:
      task352.body.selected_tranche_next_closure_packaging_follow_through_path,
    selected_tranche_next_closure_packaging_follow_through_artifact:
      task352.body.selected_tranche_next_closure_packaging_follow_through_artifact,
    delegated_selected_tranche_next_closure_packaging_results_follow_up_id:
      delegated.selected_tranche_next_closure_packaging_results_follow_up_id,
    delegated_selected_tranche_next_closure_packaging_results_follow_up_path:
      delegated.selected_tranche_next_closure_packaging_results_follow_up_path,
    delegated_selected_tranche_next_closure_packaging_results_follow_up_artifact: delegatedArtifact,
  ...
  const result: Goal3ReleaseCandidateProofBreadthSelectedTrancheNextClosurePackagingResultsClosureExecutionPackagingResultsFollowUp = {
    ...body,
    selected_tranche_next_closure_packaging_results_closure_execution_packaging_results_follow_up_artifact: {
  ...
  appendAuditEvent(projectRoot, {
    ...
    payload: {
      ...
      selected_tranche_next_closure_packaging_results_closure_execution_packaging_results_follow_up_artifact_sha256:
        result.selected_tranche_next_closure_packaging_results_closure_execution_packaging_results_follow_up_artifact
          .sha256,
      selected_tranche_next_closure_packaging_results_closure_execution_packaging_follow_through_id: task352Id,
      selected_tranche_next_closure_packaging_results_closure_execution_packaging_follow_through_artifact_sha256:
        task352.artifact.sha256,
      delegated_selected_tranche_next_closure_packaging_results_follow_up_id:
        result.delegated_selected_tranche_next_closure_packaging_results_follow_up_id,
      delegated_selected_tranche_next_closure_packaging_results_follow_up_artifact_sha256:
        result.delegated_selected_tranche_next_closure_packaging_results_follow_up_artifact.sha256,
      selected_tranche_next_closure_packaging_follow_through_id:
        result.selected_tranche_next_closure_packaging_follow_through_id,
