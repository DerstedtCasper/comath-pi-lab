...
`executeOwnedToolProcess` uses the existing native Job/process-group session with a separate admitted tool identity. It writes the tool handle without replacing a worker handle, bounds both output streams and preserves actual cancellation/timeout/termination state. Callers retain responsibility for tool charging and releasing permits only after confirmed termination. The async Lean command and version wrappers use native executables. `runServiceOwnedLeanCommandV3Async` fixes input/binary hashes and a service LRUN identity before execution, awaits outside writer transactions, then commits immutable logs, manifest and provenance. The durable proof-workflow owner consumes these helpers for per-claim/PO/RPLY workspaces, async clean replay, and final-authority evidence packaging; it rechecks current scope after awaits and only integrates after existing evidence gates pass. Changed inputs, interrupted/unconfirmed execution, or successful non-promotional evidence cannot acquire proof authority. The old synchronous path remains offline maintenance, not the managed research workflow.
