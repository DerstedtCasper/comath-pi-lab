...
Configured Codex workers use a private generation-specific `CODEX_HOME`, strict configuration validation and environment-variable credentials. Worker MCP receives its scoped service capability, not the provider credential. `sandbox_mode: "deferred"` and the historical `"native"` mode remain fail-closed before private configuration or credential materialization. The only launchable isolation mode is explicit `"oci"`: it requires an absolute Docker engine, immutable image digest, numeric non-root container user, resource limits, and a container-reachable worker listener. It runs a fixed non-privileged, read-only-root container with only generation workspace/context/runtime mounts; live engine/image/negative-isolation evidence remains a separate acceptance lane.
