...
function restoreOperatorServiceTransportClosureReviewPublicFlags(
  sanitized: unknown,
  original: unknown,
  context: string[] = []
): unknown {
  if (Array.isArray(sanitized)) {
    const originalArray = Array.isArray(original) ? original : [];
    return sanitized.map((item, index) =>
      restoreOperatorServiceTransportClosureReviewPublicFlags(item, originalArray[index], [...context, String(index)])
    );
  }
  if (!sanitized || typeof sanitized !== "object" || Array.isArray(sanitized)) {
    return sanitized;
  ...
  const restored = Object.fromEntries(
    Object.entries(sanitized as Record<string, unknown>).map(([key, item]) => [
      key,
      restoreOperatorServiceTransportClosureReviewPublicFlags(item, originalRecord[key], [...context, key])
    ])
  );
  const isTransportClosureReview =
    context.includes("transport_closure_review") ||
    originalRecord.schema_version === "comath.pi_codex_operator_service_transport_closure_review.v1";
  if (isTransportClosureReview) {
    for (const key of [
      "completion_certificate_available",
      "completionCertificateAvailable",
      "terminal_unattended_completion_certified",
      ...
      "unattended_real_host_execution_completed",
      "unattendedRealHostExecutionCompleted",
      "service_owned_checkpoint_chain_reviewed",
      "serviceOwnedCheckpointChainReviewed",
      "service_owned_durable_transport_prerequisite_configured",
      "serviceOwnedDurableTransportPrerequisiteConfigured",
      "maintained_transport_primitive_bound",
      "maintainedTransportPrimitiveBound",
      "service_route_bound",
      "serviceRouteBound",
      "client_fetch_contract_bound",
      "clientFetchContractBound",
      "terminal_completion_certificate_bound",
      "terminalCompletionCertificateBound",
      "durable_transport_contract_bound",
      "durableTransportContractBound",
      "transport_continuity_bound",
      "transportContinuityBound",
      "transport_contract_bound",
      "transportContractBound"
    ]) {
      if (originalRecord[key] === true) {
        restored[key] = true;
      }
...
function sanitizeOperatorServiceTransportClosureReviewPublicValue(value: unknown): unknown {
...
function restoreGoal3GaCertificationReviewPublicFlags(
  sanitized: unknown,
  original: unknown,
  context: string[] = []
): unknown {
  if (Array.isArray(sanitized)) {
    const originalArray = Array.isArray(original) ? original : [];
    return sanitized.map((item, index) =>
      restoreGoal3GaCertificationReviewPublicFlags(item, originalArray[index], [...context, String(index)])
    );
  }
  if (!sanitized || typeof sanitized !== "object" || Array.isArray(sanitized)) {
    return sanitized;
  ...
  const restored = Object.fromEntries(
    Object.entries(sanitized as Record<string, unknown>).map(([key, item]) => [
      key,
      restoreGoal3GaCertificationReviewPublicFlags(item, originalRecord[key], [...context, key])
    ])
  );
  const isGaCertificationReview =
    context.includes("ga_certification_review") ||
    originalRecord.schema_version === "comath.goal3_ga_certification_review.v1";
  if (isGaCertificationReview) {
    for (const key of [
      "operational_readiness_review_current",
      "operationalReadinessReviewCurrent",
      "acceptance_report_current",
      "acceptanceReportCurrent",
      "trust_core_negative_suite_fail_closed",
      "trustCoreNegativeSuiteFailClosed",
      "positive_workflow_representative_verified",
      "positiveWorkflowRepresentativeVerified",
      "ga_certification_gate_separate",
      "gaCertificationGateSeparate"
    ]) {
      if (originalRecord[key] === true) {
...
function sanitizeGoal3GaCertificationReviewPublicValue(value: unknown): unknown {
  return restoreGoal3GaCertificationReviewPublicFlags(sanitizePublicDisplayValue(value), value);
}

function restoreGoal3FinalGaAuditPublicFlags(
  sanitized: unknown,
  original: unknown,
  context: string[] = []
): unknown {
  if (Array.isArray(sanitized)) {
    const originalArray = Array.isArray(original) ? original : [];
    return sanitized.map((item, index) =>
      restoreGoal3FinalGaAuditPublicFlags(item, originalArray[index], [...context, String(index)])
    );
  }
  if (!sanitized || typeof sanitized !== "object" || Array.isArray(sanitized)) {
    return sanitized;
  ...
  const restored = Object.fromEntries(
    Object.entries(sanitized as Record<string, unknown>).map(([key, item]) => [
      key,
      restoreGoal3FinalGaAuditPublicFlags(item, originalRecord[key], [...context, key])
    ])
  );
  const isFinalGaAudit =
  ...
  if (isFinalGaAudit) {
    ...
    for (const key of [
      ...
      "ga_certification_review_current",
      "gaCertificationReviewCurrent",
      "operational_readiness_review_current",
      "operationalReadinessReviewCurrent",
      "acceptance_report_current",
      "acceptanceReportCurrent",
...
function sanitizeGoal3FinalGaAuditPublicValue(value: unknown): unknown {
  const sanitized = sanitizePublicDisplayValue(value);
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return sanitized;
  }
  const originalRecord = value as Record<string, unknown>;
  if (!Object.hasOwn(originalRecord, "final_ga_audit")) {
    return sanitized;
  }
  ...
  return {
    ...sanitizedRecord,
    final_ga_audit: restoreGoal3FinalGaAuditPublicFlags(
      sanitizePublicDisplayValue(originalRecord.final_ga_audit),
      originalRecord.final_ga_audit,
      ["final_ga_audit"]
    )
  };
}
...
function restoreGoal3ProofBreadthReviewPublicFlags(sanitized: unknown, original: unknown): unknown {
  if (!sanitized || typeof sanitized !== "object" || Array.isArray(sanitized)) {
    return sanitized;
  }
  const originalRecord =
  ...
  restored.proof_authority = "none";
  restored.proofAuthority = "none";
  restored.can_promote_claim = false;
  restored.canPromoteClaim = false;
  restored.can_certify_ga = false;
  restored.canCertifyGa = false;
  restored.proof_breadth_complete = false;
  restored.proofBreadthComplete = false;
  restored.final_ga_audit_unblocked = false;
  restored.finalGaAuditUnblocked = false;
  if (typeof originalRecord.clean_replay_passed_count === "number") {
    restored.clean_replay_passed_count = 0;
  }
  if (typeof originalRecord.promoted_count === "number") {
    restored.promoted_count = 0;
  }
  if (typeof originalRecord.replayable_blocker_count === "number") {
    restored.replayable_blocker_count =
      typeof originalRecord.total_required_tasks === "number"
        ? originalRecord.total_required_tasks
        : originalRecord.replayable_blocker_count;
  }
  for (const key of [
    "positive_task_manifest_current",
    "positiveTaskManifestCurrent",
    "positive_matrix_batch_current",
    "positiveMatrixBatchCurrent",
    "ga_certification_gate_separate",
    "gaCertificationGateSeparate"
  ]) {
    if (originalRecord[key] === true) {
...
function sanitizeGoal3ProofBreadthReviewPublicValue(value: unknown): unknown {
  const sanitized = sanitizePublicDisplayValue(value);
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return sanitized;
  }
  const originalRecord = value as Record<string, unknown>;
  if (!Object.hasOwn(originalRecord, "proof_breadth_review")) {
    return sanitized;
  }
  const sanitizedRecord =
    sanitized && typeof sanitized === "object" && !Array.isArray(sanitized)
  ...
  return {
    ...sanitizedRecord,
    proof_breadth_review: restoreGoal3ProofBreadthReviewPublicFlags(
      sanitizePublicDisplayValue(originalRecord.proof_breadth_review),
      originalRecord.proof_breadth_review
    )
  };
}
...
async function publicToolResult(name: string, result: Promise<any>): Promise<any> {
  ...
  if (name === "comath.release.goal3GaCertificationReview") {
    return sanitizeGoal3GaCertificationReviewPublicValue(value);
  }
...
const publicGoal3GaOperationalReadinessArtifactPathPattern =
...
function piLifecycleAttemptArtifactPathText(value: string, attemptId: string): string {
  const sanitized = serviceArtifactPathText(value).trim();
  const match = sanitized.match(publicPiLifecycleArtifactPathPattern);
  if (!match) {
    return sanitized;
...
function piLifecycleCompletionCertificationPrerequisiteArtifactPathText(
  value: string,
  ...
  if (
    match[1] !== completionCertificationPrerequisiteId ||
    match[2] !== "unattended-real-host-completion-certification-prerequisite.json"
  ) {
    throw new Error(
...
function piLifecycleTerminalCompletionCertificateArtifactPathText(
  value: string,
  terminalCompletionCertificateId: string
): string {
  const sanitized = serviceArtifactPathText(value).trim();
  const match = sanitized.match(publicPiLifecycleArtifactPathPattern);
  if (!match) {
  ...
  if (match[1] !== terminalCompletionCertificateId || match[2] !== "terminal-completion-certificate.json") {
    throw new Error(
      "terminal_completion_certificate_path must reference service-owned-pi-lifecycle/<terminal_completion_certificate_id>/terminal-completion-certificate.json"
    );
  }
  return `${trustedRuntimeRootName}/release/pi-codex-lifecycle/${match[1]}/${match[2]}`;
}
...
function goal3GaOperationalReadinessReviewArtifactPathText(
  value: string,
  operationalReadinessReviewId: string
): string {
  const sanitized = serviceArtifactPathText(value).trim();
  const match = sanitized.match(publicGoal3GaOperationalReadinessArtifactPathPattern);
  if (!match) {
    return sanitized;
  }
  if (match[1] !== operationalReadinessReviewId || match[2] !== "review.json") {
    throw new Error(
      "operational_readiness_review_path must reference service-owned-goal3-ga-operational-readiness/<operational_readiness_review_id>/review.json"
    );
  }
  return `${trustedRuntimeRootName}/release/goal3-ga-operational-readiness/${match[1]}/${match[2]}`;
}

function goal3GaCertificationReviewArtifactPathText(value: string, gaCertificationReviewId: string): string {
  const sanitized = serviceArtifactPathText(value).trim();
  const match = sanitized.match(publicGoal3GaCertificationArtifactPathPattern);
  if (!match) {
    return sanitized;
  }
  if (match[1] !== gaCertificationReviewId || match[2] !== "review.json") {
    throw new Error(
      "ga_certification_review_path must reference service-owned-goal3-ga-certification/<ga_certification_review_id>/review.json"
    );
  }
...
function buildPiCodexLifecycleInteractiveRealPi(input: Record<string, unknown>): Record<string, unknown> {
  ...
  const attemptId = optionalPublicPlannerToken(
    input,
    "attempt_id",
    `${sessionId}-EXECUTION-ATTEMPT`
  );
  const attemptReviewId = optionalPublicPlannerToken(
    input,
    "attempt_review_id",
  ...
  const terminalCompletionCertificateDesignId = optionalPublicPlannerToken(
    input,
  ...
  const transportClosureReviewId = optionalPublicPlannerToken(
    input,
  ...
  const gaCertificationReviewId = optionalPublicPlannerToken(
    input,
    "ga_certification_review_id",
    `${projectId}-GOAL3-GA-CERTIFICATION-REVIEW`
  );
  const finalGaAuditId = optionalPublicPlannerToken(
    input,
    "final_ga_audit_id",
  ...
  const proofBreadthReviewId = optionalPublicPlannerToken(
    input,
  ...
  const readinessPath = optionalPublicPlannerPath(
    input,
    "readiness_path",
    `service-owned-pi-lifecycle/${readinessId}/unattended-real-host-execution-readiness.json`
  );
  const readinessSha256 = optionalPublicPlannerToken(input, "readiness_sha256", "EXECUTION-READINESS-SHA256");
  const attemptPath = optionalPublicPlannerPath(
    input,
    "attempt_path",
  ...
  const completionCertificationPrerequisitePath = optionalPublicPlannerPath(
    input,
    "completion_certification_prerequisite_path",
    `service-owned-pi-lifecycle/${completionCertificationPrerequisiteId}/unattended-real-host-completion-certification-prerequisite.json`
  );
  const completionCertificationPrerequisiteSha256 = optionalPublicPlannerToken(
    input,
  ...
  const terminalCompletionCertificatePath = optionalPublicPlannerPath(
    input,
    "terminal_completion_certificate_path",
    `service-owned-pi-lifecycle/${terminalCompletionCertificateId}/terminal-completion-certificate.json`
  );
  const terminalCompletionCertificateSha256 = optionalPublicPlannerToken(
    input,
  ...
  const operationalReadinessReviewPath = optionalPublicPlannerPath(
    input,
    "operational_readiness_review_path",
    `service-owned-goal3-ga-operational-readiness/${operationalReadinessReviewId}/review.json`
  );
  const operationalReadinessReviewSha256 = optionalPublicPlannerToken(
    input,
    "operational_readiness_review_sha256",
    "GOAL3-GA-OPERATIONAL-READINESS-SHA256"
  );
  const gaCertificationReviewPath = optionalPublicPlannerPath(
    input,
    "ga_certification_review_path",
    `service-owned-goal3-ga-certification/${gaCertificationReviewId}/review.json`
  );
  const gaCertificationReviewSha256 = optionalPublicPlannerToken(
    input,
  ...
  const commandByStep: Record<typeof PI_LIFECYCLE_INTERACTIVE_REAL_PI_STEPS[number], string> = {
    ...
    "lifecycle-unattended-real-host-execution-attempt":
      `/cm:release lifecycle-unattended-real-host-execution-attempt --project-id ${projectId} ` +
      `--attempt-id ${attemptId} --readiness-id ${readinessId} ` +
      `--readiness-path ${readinessPath} --readiness-sha256 ${readinessSha256}`,
    "lifecycle-unattended-real-host-execution-attempt-review":
    ...
    "lifecycle-unattended-real-host-terminal-completion-certificate-design":
      `/cm:release lifecycle-unattended-real-host-terminal-completion-certificate-design --project-id ${projectId} ` +
    ...
    "lifecycle-operator-service-transport-closure-review":
      `/cm:release lifecycle-operator-service-transport-closure-review --project-id ${projectId} ` +
    ...
    "goal3-ga-certification-review":
      `/cm:release goal3-ga-certification-review --project-id ${projectId} ` +
      `--ga-certification-review-id ${gaCertificationReviewId} ` +
      `--operational-readiness-review-id ${operationalReadinessReviewId} ` +
      `--operational-readiness-review-path ${operationalReadinessReviewPath} ` +
      `--operational-readiness-review-sha256 ${operationalReadinessReviewSha256} ` +
      "--requested-review-mode open_formal_workbench_ga_certification",
    "goal3-final-ga-audit":
      `/cm:release goal3-final-ga-audit --project-id ${projectId} ` +
      `--final-ga-audit-id ${finalGaAuditId} ` +
      `--ga-certification-review-id ${gaCertificationReviewId} ` +
  ...
  return {
    ...
    checkpoint_inputs: {
      ...
      operational_readiness_review_path: operationalReadinessReviewPath,
...
export async function executeComathTool(client: ComathClient, name: string, input: Record<string, unknown>): Promise<any> {
  ...
  if (name === "comath.release.piCodexLifecycleUnattendedRealHostExecutionAttempt") {
    const attemptId = readString(input, "attempt_id", { optional: true });
    const requestedExecutionMode =
      readString(input, "requested_execution_mode", { optional: true }) ?? "production_unattended_real_host";
    return publicToolResult(
      name,
      client.post("/release/pi-codex-lifecycle/unattended-real-host-execution-attempt", {
        project_root: readString(input, "project_root"),
        project_id: readString(input, "project_id"),
        actor: publicOperatorText(readString(input, "actor")),
        ...(attemptId === undefined ? {} : { attempt_id: publicOperatorText(attemptId) }),
        readiness_id: readString(input, "readiness_id"),
        readiness_path: piLifecycleCanonicalArtifactPathText(readString(input, "readiness_path")),
        readiness_sha256: readString(input, "readiness_sha256"),
        requested_execution_mode: requestedExecutionMode
      })
    );
  }

  if (name === "comath.release.piCodexLifecycleUnattendedRealHostExecutionAttemptReview") {
    const attemptReviewId = readString(input, "attempt_review_id", { optional: true });
    const attemptId = readString(input, "attempt_id");
    const requestedReviewMode =
      readString(input, "requested_review_mode", { optional: true }) ?? "terminal_unattended_real_host_execution";
    return publicToolResult(
      name,
      client.post("/release/pi-codex-lifecycle/unattended-real-host-execution-attempt-review", {
        project_root: readString(input, "project_root"),
        project_id: readString(input, "project_id"),
        actor: publicOperatorText(readString(input, "actor")),
        ...(attemptReviewId === undefined ? {} : { attempt_review_id: publicOperatorText(attemptReviewId) }),
        attempt_id: attemptId,
        attempt_path: piLifecycleAttemptArtifactPathText(readString(input, "attempt_path"), attemptId),
        attempt_sha256: readString(input, "attempt_sha256"),
  ...
  if (name === "comath.release.piCodexLifecycleUnattendedRealHostTerminalCompletionCertificateDesign") {
    ...
    return publicToolResult(
      name,
      client.post("/release/pi-codex-lifecycle/unattended-real-host-terminal-completion-certificate-design", {
        project_root: readString(input, "project_root"),
        project_id: readString(input, "project_id"),
        actor: publicOperatorText(readString(input, "actor")),
        ...(terminalCompletionCertificateDesignId === undefined
          ? {}
          : {
              terminal_completion_certificate_design_id: publicOperatorText(
                terminalCompletionCertificateDesignId
              )
            }),
        completion_certification_prerequisite_id: completionCertificationPrerequisiteId,
        completion_certification_prerequisite_path: piLifecycleCompletionCertificationPrerequisiteArtifactPathText(
          readString(input, "completion_certification_prerequisite_path"),
          completionCertificationPrerequisiteId
        ),
  ...
  if (name === "comath.release.piCodexLifecycleOperatorServiceTransportClosureReview") {
    ...
    const terminalCompletionCertificateId = readString(input, "terminal_completion_certificate_id");
    const requestedTransportClosureMode =
      readString(input, "requested_transport_closure_mode", { optional: true }) ??
      "maintained_operator_service_transport_closure_review";
    return publicToolResult(
      name,
      client.post("/release/pi-codex-lifecycle/operator-service-transport-closure-review", {
        project_root: readString(input, "project_root"),
        project_id: readString(input, "project_id"),
        actor: publicOperatorText(readString(input, "actor")),
        ...(transportClosureReviewId === undefined
          ? {}
          : {
              transport_closure_review_id: publicOperatorText(transportClosureReviewId)
            }),
        terminal_completion_certificate_id: terminalCompletionCertificateId,
        terminal_completion_certificate_path: piLifecycleTerminalCompletionCertificateArtifactPathText(
          readString(input, "terminal_completion_certificate_path"),
  ...
  if (name === "comath.release.goal3GaCertificationReview") {
    ...
    return publicToolResult(
      name,
      client.post("/release/goal3/ga-certification-review", {
        project_root: readString(input, "project_root"),
        project_id: readString(input, "project_id"),
        actor: publicOperatorText(readString(input, "actor")),
        ...(gaCertificationReviewId === undefined
          ? {}
          : {
              ga_certification_review_id: publicOperatorText(gaCertificationReviewId)
            }),
        operational_readiness_review_id: operationalReadinessReviewId,
        operational_readiness_review_path: goal3GaOperationalReadinessReviewArtifactPathText(
          readString(input, "operational_readiness_review_path"),
          operationalReadinessReviewId
        ),
        operational_readiness_review_sha256: readString(input, "operational_readiness_review_sha256"),
        requested_review_mode: requestedReviewMode
      })
    );
  }
  ...
  if (name === "comath.release.goal3FinalGaAudit") {
    const finalGaAuditId = readString(input, "final_ga_audit_id", {
      optional: true
    });
    const gaCertificationReviewId = readString(input, "ga_certification_review_id");
    ...
    return publicToolResult(
      name,
      client.post("/release/goal3/final-ga-audit", {
        project_root: readString(input, "project_root"),
        project_id: readString(input, "project_id"),
        actor: publicOperatorText(readString(input, "actor")),
        ...(finalGaAuditId === undefined
          ? {}
          : {
              final_ga_audit_id: publicOperatorText(finalGaAuditId)
            }),
        ga_certification_review_id: gaCertificationReviewId,
        ga_certification_review_path: goal3GaCertificationReviewArtifactPathText(
          readString(input, "ga_certification_review_path"),
  ...
  if (name === "comath.release.goal3ProofBreadthReview") {
    const proofBreadthReviewId = readString(input, "proof_breadth_review_id", {
      optional: true
    });
    const requestedReviewMode =
      readString(input, "requested_review_mode", { optional: true }) ??
    ...
    return publicToolResult(
      name,
      client.post("/release/goal3/proof-breadth-review", {
        project_root: readString(input, "project_root"),
        project_id: readString(input, "project_id"),
        actor: publicOperatorText(readString(input, "actor")),
        ...(proofBreadthReviewId === undefined
          ? {}
          : {
...
export function createComathTools(): ToolDescriptor[] {
  ...
  return [
    ...
    },
    {
      name: "comath.release.piCodexLifecycleUnattendedRealHostExecutionAttempt",
      description:
        "Record a host-confirmed service-owned unattended real-host execution attempt blocker through comathd using readiness id/path/hash, without exposing executor commands, terminal completion, proof authority, GA certification, direct Pi mutation, or durable/live transport claims.",
      mutates: true,
      input_schema: requireConfirmationSchema(
        objectSchema(
          [
          ...
          ],
          {
            ...
            project_id: stringProp,
            actor: stringProp,
            attempt_id: stringProp,
            readiness_id: stringProp,
            readiness_path: stringProp,
            readiness_sha256: stringProp,
            requested_execution_mode: {
              type: "string",
              enum: ["production_unattended_real_host"]
            }
    ...
    },
    {
      name: "comath.release.piCodexLifecycleUnattendedRealHostExecutionAttemptReview",
      description:
        "Record a host-confirmed service-owned unattended real-host execution attempt review through comathd using attempt id/path/hash, without exposing executor commands, caller attempt results, terminal completion, proof authority, GA certification, direct Pi mutation, or durable/live transport claims.",
      mutates: true,
      input_schema: requireConfirmationSchema(
        objectSchema(
          [
          ...
          ],
          {
            ...
            attempt_id: stringProp,
            attempt_path: stringProp,
            attempt_sha256: stringProp,
    ...
    },
    {
      name: "comath.release.piCodexLifecycleUnattendedRealHostTerminalCompletionCertificateDesign",
      description:
        "Record a host-confirmed service-owned unattended real-host terminal completion certificate design through comathd using completion-prerequisite id/path/hash, without exposing executor commands, caller attempt results, completion certificates, terminal completion, proof authority, GA certification, direct Pi mutation, or durable/live transport claims.",
      mutates: true,
      input_schema: requireConfirmationSchema(
        objectSchema(
          [
    ...
    },
    {
      name: "comath.release.piCodexLifecycleOperatorServiceTransportClosureReview",
      description:
        "Record a host-confirmed Pi consumer bridge for the service-owned operator/service transport closure review using terminal-completion-certificate id/path/hash, without exposing executor commands, caller attempt results, caller completion certificates, Lean proof authority, GA certification, direct Pi mutation, or durable/live transport claims.",
      mutates: true,
      input_schema: requireConfirmationSchema(
        objectSchema(
          [
          ...
          ],
          {
            ...
            terminal_completion_certificate_id: stringProp,
            terminal_completion_certificate_path: stringProp,
            terminal_completion_certificate_sha256: stringProp,
      ...
      input_schema: requireConfirmationSchema(
        objectSchema(
          ...
          ],
          {
            ...
            operational_readiness_review_id: stringProp,
            operational_readiness_review_path: stringProp,
            operational_readiness_review_sha256: stringProp,
            requested_review_mode: {
              type: "string",
              enum: ["open_formal_workbench_ga_certification"]
            }
          }
        )
      )
      ...
      input_schema: requireConfirmationSchema(
        objectSchema(
          ...
          ],
          {
            ...
            project_id: stringProp,
            actor: stringProp,
            proof_breadth_review_id: stringProp,
            requested_review_mode: {
              type: "string",
...
async function handleReleaseCommand(
  ...
  if (subcommand === "lifecycle-unattended-real-host-execution-attempt") {
    const tool = createComathTools().find(
      (descriptor) => descriptor.name === "comath.release.piCodexLifecycleUnattendedRealHostExecutionAttempt"
    );
    if (!tool) {
      throw new Error("Pi/Codex lifecycle unattended real-host execution attempt tool is not registered");
    }
    await notifyRuntimeResult(
      ctx,
      await executeRuntimeToolWithHostConfirmation(
        ...
        tool,
        {
          ...
          project_id: requiredOption(optionValue(parsed.args, "--project-id"), "project_id"),
          actor: actorFrom(options, parsed.args),
          attempt_id: optionValue(parsed.args, "--attempt-id"),
          readiness_id: requiredOption(optionValue(parsed.args, "--readiness-id"), "readiness_id"),
          readiness_path: requiredOption(optionValue(parsed.args, "--readiness-path"), "readiness_path"),
          readiness_sha256: requiredOption(optionValue(parsed.args, "--readiness-sha256"), "readiness_sha256"),
          requested_execution_mode:
            optionValue(parsed.args, "--requested-execution-mode") ?? "production_unattended_real_host"
        },
        ctx
  ...
  if (subcommand === "lifecycle-unattended-real-host-execution-attempt-review") {
    const tool = createComathTools().find(
      (descriptor) => descriptor.name === "comath.release.piCodexLifecycleUnattendedRealHostExecutionAttemptReview"
    );
    if (!tool) {
      throw new Error("Pi/Codex lifecycle unattended real-host execution attempt review tool is not registered");
    }
    await notifyRuntimeResult(
      ctx,
      await executeRuntimeToolWithHostConfirmation(
        ...
        tool,
        {
          ...
          project_id: requiredOption(optionValue(parsed.args, "--project-id"), "project_id"),
          actor: actorFrom(options, parsed.args),
          attempt_review_id: optionValue(parsed.args, "--attempt-review-id"),
          attempt_id: requiredOption(optionValue(parsed.args, "--attempt-id"), "attempt_id"),
          attempt_path: requiredOption(optionValue(parsed.args, "--attempt-path"), "attempt_path"),
  ...
  if (subcommand === "lifecycle-unattended-real-host-terminal-completion-certificate-design") {
    const tool = createComathTools().find(
      (descriptor) =>
        descriptor.name ===
        "comath.release.piCodexLifecycleUnattendedRealHostTerminalCompletionCertificateDesign"
    ...
    if (!tool) {
      throw new Error(
        "Pi/Codex lifecycle unattended real-host terminal completion certificate design tool is not registered"
      );
    }
    await notifyRuntimeResult(
      ctx,
      await executeRuntimeToolWithHostConfirmation(
        ...
        tool,
        {
          ...
          project_id: requiredOption(optionValue(parsed.args, "--project-id"), "project_id"),
          actor: actorFrom(options, parsed.args),
          terminal_completion_certificate_design_id: optionValue(
            parsed.args,
            "--terminal-completion-certificate-design-id"
          ),
          completion_certification_prerequisite_id: requiredOption(
            optionValue(parsed.args, "--completion-certification-prerequisite-id"),
            "completion_certification_prerequisite_id"
          ),
          completion_certification_prerequisite_path: requiredOption(
            optionValue(parsed.args, "--completion-certification-prerequisite-path"),
            "completion_certification_prerequisite_path"
          ),
          completion_certification_prerequisite_sha256: requiredOption(
            optionValue(parsed.args, "--completion-certification-prerequisite-sha256"),
            "completion_certification_prerequisite_sha256"
          ),
          requested_certificate_mode:
  ...
  if (subcommand === "lifecycle-operator-service-transport-closure-review") {
    const tool = createComathTools().find(
      (descriptor) =>
        descriptor.name === "comath.release.piCodexLifecycleOperatorServiceTransportClosureReview"
    );
    if (!tool) {
      throw new Error(
        "Pi/Codex lifecycle operator/service transport closure review tool is not registered"
      );
    }
    await notifyRuntimeResult(
      ctx,
      await executeRuntimeToolWithHostConfirmation(
        ...
        tool,
        {
          ...
          project_id: requiredOption(optionValue(parsed.args, "--project-id"), "project_id"),
          actor: actorFrom(options, parsed.args),
          transport_closure_review_id: optionValue(parsed.args, "--transport-closure-review-id"),
          terminal_completion_certificate_id: requiredOption(
            optionValue(parsed.args, "--terminal-completion-certificate-id"),
            "terminal_completion_certificate_id"
          ),
          terminal_completion_certificate_path: requiredOption(
            optionValue(parsed.args, "--terminal-completion-certificate-path"),
            "terminal_completion_certificate_path"
          ),
          terminal_completion_certificate_sha256: requiredOption(
            optionValue(parsed.args, "--terminal-completion-certificate-sha256"),
            "terminal_completion_certificate_sha256"
          ),
          requested_transport_closure_mode:
  ...
  if (subcommand === "goal3-ga-certification-review") {
    const tool = createComathTools().find(
    ...
    await notifyRuntimeResult(
      ...
      await executeRuntimeToolWithHostConfirmation(
        ...
        tool,
        {
          ...
          project_id: requiredOption(optionValue(parsed.args, "--project-id"), "project_id"),
          actor: actorFrom(options, parsed.args),
          ga_certification_review_id: optionValue(parsed.args, "--ga-certification-review-id"),
          operational_readiness_review_id: requiredOption(
            optionValue(parsed.args, "--operational-readiness-review-id"),
            "operational_readiness_review_id"
          ),
          operational_readiness_review_path: requiredOption(
            optionValue(parsed.args, "--operational-readiness-review-path"),
            "operational_readiness_review_path"
          ),
          operational_readiness_review_sha256: requiredOption(
            optionValue(parsed.args, "--operational-readiness-review-sha256"),
            "operational_readiness_review_sha256"
          ),
          requested_review_mode:
            optionValue(parsed.args, "--requested-review-mode") ??
  ...
  if (subcommand === "goal3-final-ga-audit") {
    const tool = createComathTools().find(
    ...
    await notifyRuntimeResult(
      ...
      await executeRuntimeToolWithHostConfirmation(
        ...
        tool,
        {
          ...
          project_id: requiredOption(optionValue(parsed.args, "--project-id"), "project_id"),
          actor: actorFrom(options, parsed.args),
          final_ga_audit_id: optionValue(parsed.args, "--final-ga-audit-id"),
          ga_certification_review_id: requiredOption(
            optionValue(parsed.args, "--ga-certification-review-id"),
            "ga_certification_review_id"
          ),
          ga_certification_review_path: requiredOption(
            optionValue(parsed.args, "--ga-certification-review-path"),
            "ga_certification_review_path"
          ),
          ga_certification_review_sha256: requiredOption(
  ...
  if (subcommand === "goal3-proof-breadth-review") {
    const tool = createComathTools().find(
    ...
    await notifyRuntimeResult(
      ...
      await executeRuntimeToolWithHostConfirmation(
        ...
        tool,
        {
          ...
          project_id: requiredOption(optionValue(parsed.args, "--project-id"), "project_id"),
          actor: actorFrom(options, parsed.args),
          proof_breadth_review_id: optionValue(parsed.args, "--proof-breadth-review-id"),
          requested_review_mode:
            optionValue(parsed.args, "--requested-review-mode") ??
            "open_formal_workbench_release_candidate_proof_breadth"
        },
