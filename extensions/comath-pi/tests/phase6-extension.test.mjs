...
assert.equal(
  tools.some(
    (tool) =>
      tool.name === "comath.release.piCodexLifecycleUnattendedRealHostCompletionCertificationPrerequisite"
  ),
  true
);
assert.equal(
  tools.some((tool) => tool.name === "comath.release.piCodexLifecycleUnattendedRealHostTerminalCompletionCertificate"),
  true
);
assert.equal(tools.some((tool) => tool.name === "comath.release.goal3GaOperationalReadinessReview"), true);
assert.equal(tools.some((tool) => tool.name === "comath.release.goal3GaCertificationReview"), true);
assert.equal(tools.some((tool) => tool.name === "comath.release.goal3FinalGaAudit"), true);
assert.equal(
  tools.some((tool) => tool.name === "comath.release.goal3SourceReleaseOsImmutabilityAttestation"),
  true
);
assert.equal(
  tools.some((tool) => tool.name === "comath.release.goal3FinalReleaseCandidateClosureAudit"),
  true
);
