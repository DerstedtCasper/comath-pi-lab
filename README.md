...
Pi/Codex lifecycle release evidence now has service-owned non-authoritative producers for artifact-backed lifecycle intake, durable `comathd` service lifecycle probes, and production Codex API account/network validation probes, with Pi release tooling for the lifecycle review and Codex API probe routes. These reports can feed the lifecycle readiness gate, but they cannot promote claims, certify GA, expose API secrets, or replace real Pi host install/runtime-registration validation.
