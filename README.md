...
Pi/Codex lifecycle release evidence now has service-owned non-authoritative producers for artifact-backed lifecycle intake, durable `comathd` service lifecycle probes, production Codex API account/network validation probes, real-Pi install/runtime-registration probe artifacts, durable operator-session manifests, and operator transport recovery checkpoint artifacts, with Pi release tooling for the lifecycle review, Codex API probe, real-Pi runtime probe, read-only lifecycle walkthrough, bounded `lifecycle-control` operator actions, read-only `lifecycle-session` recovery planning, and host-confirmed `lifecycle-operator-session` service persistence. These reports, manifests, checkpoints, walkthroughs, controls, and recovery plans can feed or guide the lifecycle readiness gate, but they cannot promote claims, certify GA, expose API secrets, claim live durable long-lived transport, or replace an operator-controlled real Pi host run.
