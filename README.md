...
Task203 tightens complete provider-helper collection evidence: complete OS-enforcement facts now also require a provider-specific executed-tool witness bound to service-derived collection-probe executable/profile/fixed-argv-template hashes, the current collection, helper execution, runner, launch, provider, disabled-network policy, and helper transcript hash. Missing, mismatched, or stale pre-Task203 witness material becomes a replayable blocker, including for internal service-owned callbacks; caller route payloads still cannot provide readiness evidence, and the wrapper remains non-authoritative.
