...
Pi/Codex lifecycle release evidence now has service-owned non-authoritative producers for artifact-backed lifecycle intake, durable `comathd` service lifecycle probes, production Codex API account/network validation probes, and real-Pi install/runtime-registration probe artifacts, with Pi release tooling for the lifecycle review, Codex API probe, real-Pi runtime probe, read-only lifecycle walkthrough, and bounded `lifecycle-control` operator actions. These reports, walkthroughs, and host-confirmed controls can feed or guide the lifecycle readiness gate, but they cannot promote claims, certify GA, expose API secrets, or replace an operator-controlled real Pi host run.
